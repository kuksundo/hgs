...
function LRTrim_CRLF( const s: string ): String;
...
Function Dec2HexStr(n: integer): String;
...
begin
  ...
  if s <> '' then
  begin
    while (i < 1000) and (s[i] in [#10,#13]) do
      inc(i);

    str1 := copy(s, i, length(s));
    i := Length(str1);

    while (i >0) and (str1[i] in [#10,#13]) do
      Dec(i);
    LRTrim_CRLF := copy(str1, 1, i);
  end;
end;
