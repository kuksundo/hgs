...
uses Winapi.Windows, Winapi.Messages, WinTypes, SysUtils, Forms, UnitCATVAsyncIPC;

Const
  Author = 'JH Park';  // 저자명 : 실행여부를 검사하기 위한 검색어를 구성함.
//  RunProgram = 'ModbusComm_Kumo';    // 프로그램명 :             "
...
//      StrPCopy(AtomText, Author + RunProgram + IntToStr(Handle));
      StrPCopy(AtomText, Author + ExtractFileName(Application.ExeName) + IntToStr(Handle));
...
var
  CATVAsycIPC: TCATVAsycIPC;
begin
  CATVAsycIPC := TCATVAsycIPC.Create;
  try
    CATVAsycIPC.SendCommandToCromisIPC;
  finally
    CATVAsycIPC.Free;
  end;
end;
...
//     GCATVRecord.FFileName := ParamStr(1);
//     SendRecordCopyData(MyPopup, GCATVRecord);
...
//     StrPCopy(AtomText, Author + RunProgram + IntToStr(Application.Handle));
     StrPCopy(AtomText, Author + ExtractFileName(Application.ExeName) + IntToStr(Application.Handle));
