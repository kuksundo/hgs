...
uses
  ...
  IPCThrd_ECS_avat, DataSave2FileThread, DataSave2DBThread, SyncObjs,inifiles,
  DataSaveConfig, IPCThrdMonitor_ECS_avat, IPCThrdMonitor_Dynamo, DeCAL,
...
//    GLUserShader1: TGLUserShader;
  ...
  private
    ...
    procedure Value2Screen(BlockNo: integer; AModbusMode: integer);
  ...
  public
    ...
    FIPCMonitor_ECS_avat: TIPCMonitor_ECS_avat;//공유 메모리 및 이벤트 객체
...
procedure TDataSaveMain.ECS_OnSignal(Sender: TIPCThread_ECS_avat; Data: TEventData_ECS_avat);
var
  i,dcount: integer;
begin
...
procedure TDataSaveMain.UpdateTrace_ECS_avat(var Msg: TEventData_ECS_avat);
var
  it: DIterator;
  pHiMap: THiMap;
  i, j, BlockNo: integer;
...
begin
  ...
  begin
    ...
    begin
      if FECSData.ModBusMode = 3 then
        pHiMap.FMaxval := FECSData.InpDataBuf_double[i]
      else
  ...
  //수신된 데이타를 화면에 뿌려줌
  Value2Screen(BlockNo,FECSData.ModBusMode);
end;
//데이터의 저장
procedure TDataSaveMain.Value2Screen(BlockNo: integer; AModbusMode: integer);
var
  it: DIterator;
  pHiMap: THiMap;
  Lstr: string;
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if pHiMap.FDescription <> 'dummy' then
        begin
          if pHiMap.FAlarm then
          begin
            if AModbusMode = 3 then
            begin
              FSaveDataBuf := FSaveDataBuf+ ',' + FloatToStr(pHiMap.FMaxval);
              FSaveDBDataBuf := FSaveDBDataBuf+ ',' + FloatToStr(pHiMap.FMaxval);
            end
            else
            begin
              FSaveDataBuf := FSaveDataBuf+ ',' + FloatToStr(pHiMap.FValue * pHiMap.FMaxval);
              FSaveDBDataBuf := FSaveDBDataBuf+ ',' + FloatToStr(pHiMap.FValue * pHiMap.FMaxval);
            end;
          end
          else
          begin
...
begin
  ...
  begin
    ...
    FlogStart := False;
    FIPCMonitor_ECS_avat.Suspend;    //공유메모리 모니터쓰레드 동작 중지
