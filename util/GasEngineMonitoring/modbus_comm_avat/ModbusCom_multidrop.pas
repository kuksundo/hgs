...
uses
  ...
  Dialogs, CPort, DeCAL, IPCThrd_ECS_avat, IPCThrdClient_ECS_avat, ModbusComConst, CommonUtil,
...
type
  ...
  private
    ...
    FIPCClient2: TIPCClient_ECS_avat;//공유 메모리 및 이벤트 객체
...
begin
  ...
  FIPCClient := TIPCClient_ECS_avat.Create(0, IPCCLIENTNAME1, True);
  FIPCClient2 := TIPCClient_ECS_avat.Create(0, IPCCLIENTNAME2, True);
...
begin
  ...
  begin
    ...
    FModBusTCPThread.Suspended := True;
    FSendCommandList.AddObject(SendBuff, ModbusTCP_Command);
    FModBusTCPThread.Resume;
  end
end;
...
begin
  ...
  begin
    ...
    Lstr := '********'''+ IPCCLIENTNAME1 + ''', Send OK!(ModbusTCP : IP = ''' + FIPAddress;
