...
// ---------------------------------------------------------------------- //
//	- Description: 
//		Source file for Main DSP Built In Test
// ---------------------------------------------------------------------- //
//	- Revision History -
// ---------------------------------------------------------------------- //
//		2011-04-21:	Draft
...
int Memory_BIT (Uint32 base_address, Uint32 memory_size)
{
	...
	Memory_Save_Count(DPRAM_STR_BASE, DPRAM_STR_SIZE);
	Memory_Save_Count(DPRAM_M_BASE, DPRAM_M_SIZE);
	Memory_Save_Count(DPRAM_2808_BASE, DPRAM_2808_SIZE);
	*/
	...
	{
		case SRAM_BASE:
			other_mem_test_result += Memory_Count_Test(DPRAM_STR_BASE, DPRAM_STR_SIZE);
			other_mem_test_result += Memory_Count_Test(DPRAM_M_BASE, DPRAM_M_SIZE);
			other_mem_test_result += Memory_Count_Test(DPRAM_2808_BASE, DPRAM_2808_SIZE);
			break;
...
void Print_Memory_Error (Uint32 Address, Uint16 Data_WR, Uint16 Data_RD)
{
	...
	Printf("\n\r-Time: 0x%x%xms",Error_Time_H, Error_Time_L);
	Printf("\n\r-Address: 0x%x%x", Address_H, Address_L);
	Printf("\n\r-Expect Data: 0x%x,   -Read Data:0x%x\n\r", Data_WR, Data_RD);	
}
