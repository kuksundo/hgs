...
#ifndef MAIN_DSP_ANALOG_INPUT_H
#define MAIN_DSP_ANALOG_INPUT_H
...
typedef struct tagBitByte
{
	...
	Uint16 Bit8		:1;		//0 bit
	Uint16 Bit9		:1;		//0 bit
	Uint16 Bit10		:1;		//0 bit
	Uint16 Bit11		:1;		//0 bit
	Uint16 Bit12		:1;		//0 bit
	Uint16 Bit13		:1;		//0 bit
	Uint16 Bit14		:1;		//0 bit
	Uint16 Bit15		:1;		//0 bit
} BITBYTE, * LP_BITBYTE;

typedef union tagBitByteVar
{
	Uint16   all;	
	BITBYTE Bit;		//∫Ò∆Æ
} BITBYTEVAR, * LP_BITBYTEVAR;
// ---------------------------------------------------------------------- //

//extern volatile union BITBYTEVAR stErrorReg[20];
...
struct Analog_Input_Value {
	// Register value
	float 	Min;			// low value
	float	Max;			// high value
//	float 	set_value;				// set value
	unsigned int delay;     // delay for error
	unsigned int period;    // evaluation period
	unsigned int period_cnt; // ms count for evaluate
	unsigned int delay_cnt; // ms count for delay
	unsigned int delay_cnt_lowhigh; // ms count for delay
	unsigned int delay_cnt_warn; // ms count for delay
	unsigned int delay_cnt_GT; // ms count for delay
	unsigned int delay_cnt_PT; // ms count for delay
	unsigned int delay_cnt_Alarm; // ms count for delay
	unsigned int delay_cnt_StartBlock; // ms count for delay
	unsigned char flagLowHigh; 
	unsigned char flagWarn; 
	unsigned char flagErr; 
	unsigned char flagGT;   //Gas Trip
	unsigned char flagPT;  // Pilot Trip
	unsigned char flagAlarm;
	unsigned char flagStartBlock;
	float value;
	float value_old;
	};
extern volatile struct Analog_Input_Value st_ADC_Value[0x20];


struct Digital_Input_Value {
   	unsigned int delay_cnt_lowhigh; // ms count for delay
	unsigned int delay_cnt_warn; // ms count for delay
	unsigned int delay_cnt_GT; // ms count for delay
	unsigned int delay_cnt_PT; // ms count for delay
...
extern volatile struct Digital_Input_Value st_DI_Value[0x20];
...
#endif  // end of MAIN_DSP_ANALOG_INPUT_H definition
