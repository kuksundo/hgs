...
uses
  Windows, Classes, Messages, SysUtils, vcl.Forms;
...
type
  ...
  ENoTimersAvailable = class(Exception)
  public
    constructor Create;
  end;
...
type
  TPJHTimerPool = class(TComponent)
  protected {private}
    {property variables}
...
constructor TPJHTimerPool.Create(AOwner: TComponent);
begin
...
destructor TPJHTimerPool.Destroy;
var
  I : Integer;
begin
...
function TPJHTimerPool.AddOneShot(OnTrigger : TVpTimerTriggerEvent; Interval : Cardinal) : Integer;
  {-adds or updates one timer trigger. removed automatically after one firing}
var
  I : Integer;
begin
...
function TPJHTimerPool.AddOneTime(OnTrigger : TVpTimerTriggerEvent; Interval : Cardinal) : Integer;
  {-adds a new timer trigger. removed automatically after one firing}
var
  I : Integer;
begin
...
function TPJHTimerPool.Add(OnTrigger : TVpTimerTriggerEvent; Interval : Cardinal) : Integer;
  {-adds a new timer trigger and returns a handle}
var
  ER : PEventRec;
begin
...
procedure TPJHTimerPool.DoTriggerNotification;
...
function TPJHTimerPool.GetElapsedTriggerTime(Handle : Integer) : LongInt;
...
function TPJHTimerPool.GetElapsedTriggerTimeSec(Handle : Integer) : LongInt;
  {-return the number of seconds since the timer trigger was created}
begin
  Result := GetElapsedTriggerTime(Handle) div 1000;
end;

function TPJHTimerPool.GetOnTrigger(Handle : Integer) : TVpTimerTriggerEvent;
...
function TPJHTimerPool.GetTriggerCount : Integer;
  {-returns the number of maintained timer triggers}
begin
  Result := tpList.Count;
end;

function TPJHTimerPool.GetTriggerEnabled(Handle : Integer) : Boolean;
...
function TPJHTimerPool.GetTriggerInterval(Handle : Integer) : Cardinal;
...
procedure TPJHTimerPool.Remove(Handle : Integer);
  {-removes the timer trigger}
var
  ER : PEventRec;
  I  : Integer;
...
procedure TPJHTimerPool.RemoveAll;
  {-disable and destroy all timer triggers}
var
  ER : PEventRec;
  I  : Integer;
...
procedure TPJHTimerPool.ResetElapsedTime(Handle : Integer);
  {-resets ElapsedTime for a given Trigger to 0}
var
  I : Integer;
begin
...
procedure TPJHTimerPool.SetOnTrigger(Handle : Integer; Value: TVpTimerTriggerEvent);
...
procedure TPJHTimerPool.SetTriggerEnabled(Handle : Integer; Value: Boolean);
...
procedure TPJHTimerPool.SetTriggerInterval(Handle : Integer; Value : Cardinal);
...
procedure TPJHTimerPool.tpCalcNewInterval;
  {-calculates the needed interval for the window's timer}
var
  I    : Integer;
  N, V : LongInt;
...
function TPJHTimerPool.tpCountEnabledTriggers : Integer;
  {-returns the number of enabled/active timer triggers}
var
  I : Integer;
begin
...
function TPJHTimerPool.tpCreateTriggerHandle : Integer;
  {-returns a unique timer trigger handle}
var
  I : Integer;
  H : Integer;
...
function TPJHTimerPool.tpEventIndex(Handle : Integer) : Integer;
...
procedure TPJHTimerPool.tpSortTriggers;
  {-sorts the internal list of timer trigger event records}
var
  I    : Integer;
  Done : Boolean;
...
procedure TPJHTimerPool.tpTimerWndProc(var Msg : TMessage);
  {-window procedure to catch timer messages}
begin
  with Msg do
...
procedure TPJHTimerPool.tpUpdateTimer;
  {-re-create the windows timer with a new timer interval}
begin
...
begin
  inherited Create('No Window''s timers are available.');
end;
