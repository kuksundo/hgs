...
uses
  ...
  JvCheckListBox, DragDropText, HiMECSComponentCollect, Dialogs, NxColumnClasses,
...
type
  ...
  private
    ...
    FHiMECSComponents: THiMECSComponents;
...
begin
  ...
  begin
    ...
    begin
      j := NextGrid1.AddRow;
      //NextGrid1.Cell[2,j].AsString := ExtractFileName(ListView1.Folders[i].DisplayName);
      NextGrid1.Cell[2,j].AsString := ListView1.Folders[i].PathName;
      FCurrentCount := NextGrid1.RowCount - 1;
      LStr := ExtractFileName(NextGrid1.Cell[2,j].AsString);
      if NextGrid1.Cell[3,j].AsString = '' then
        NextGrid1.Cell[3,j].AsString := 'Create_' +  Copy(LStr,1,pos('.',LStr)-1);
...
begin
  ...
  begin
    ...
    begin
      FHiMECSComponents.FPackageCollect.Clear;

      for i := 0 to NextGrid1.RowCount - 1 do
      begin
        if NextGrid1.Cell[0,i].AsBoolean then
        begin
          with FHiMECSComponents.FPackageCollect.Add do
          begin
            AllowLevel := NextGrid1.Cell[4,i].AsInteger;
            PageName := NextGrid1.Cell[1,i].AsString;
            PackageName := NextGrid1.Cell[2,i].AsString;
            LStr := ExtractFileName(PackageName);
            if NextGrid1.Cell[2,i].AsString = '' then
              CreateFuncName := 'Create_' +  Copy(LStr,1,pos('.',LStr)-1)
            else
              CreateFuncName := NextGrid1.Cell[3,i].AsString;
          end;
        end;
      end;

      LStr := ExtractFileName(SaveDialog1.FileName);
      FHiMECSComponents.SaveToFile(SaveDialog1.FileName,LStr,True);
      FItemChanged := False;
    end
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FHiMECSComponents.LoadFromFile(OpenDialog1.FileName,LStr,True);
      NextGrid1.ClearRows;

      for i := 0 to FHiMECSComponents.FPackageCollect.Count - 1 do
      begin
        NextGrid1.AddRow;
        NextGrid1.Cell[0, i].AsBoolean := True;
        NextGrid1.Cell[1, i].AsString := FHiMECSComponents.FPackageCollect.Items[i].PageName;
        NextGrid1.Cell[2, i].AsString := FHiMECSComponents.FPackageCollect.Items[i].PackageName;
        NextGrid1.Cell[3, i].AsString := FHiMECSComponents.FPackageCollect.Items[i].CreateFuncName;
        NextGrid1.Cell[4, i].AsInteger := FHiMECSComponents.FPackageCollect.Items[i].AllowLevel;
      end;

      if FHiMECSComponents.FPackageCollect.Count > 0 then
        FItemChanged := False;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      j := NextGrid1.AddRow;
      NextGrid1.Cell[2,j].AsString := ( ExtractFileName(
          (DataFormatAdapterFile.DataFormat as TFileDataFormat).Files.Strings[i]));
      NextGrid1.Cell[0,j].AsBoolean := true; //[i+FCurrentCount]
    end;
