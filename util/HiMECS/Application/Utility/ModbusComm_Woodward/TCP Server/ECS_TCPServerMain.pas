...
uses
  ...
  IdComponent, IdStack, IPCThrd_ECS_kumo, Menus, ExtCtrls, iniFiles,
  TCPConfig, TCPServerConst, ComCtrls, SyncObjs, IPCThrdMonitor_ECS_kumo,
  IdContext, IdCustomTCPServer, JvComponentBase, JvTrayIcon, JvExControls,
  JvXPCore, JvXPButtons, SynEdit, SynMemo, UnitFrame;
...
type
  ...
  public
    ...
    FIPAddress: string;
    FEventData_ECS_kumo: TEventData_ECS_kumo; //공유메모리로부터 받은 데이터 저장(smh)
...
uses ECS_Kumo_TCPUtil, TCPServer_Util;
...
procedure TServerFrmMain.ECS_OnSignal(Sender: TIPCThread_ECS_kumo; Data: TEventData_ECS_kumo);
var
  i: Integer;
begin
  ...
  System.Move(Data, FEventData_ECS_kumo, SizeOf(TEventData_ECS_kumo));
  //DisplayMessage (TimeToStr(Time) +' move' + ': NumOfData :'+ IntToStr(FEventData_ECS_kumo.NumOfData), dtSendMemo);
...
begin
  LStream := TMemoryStream.Create;
  WriteEventData2Stream(FEventData_ECS_kumo, LStream);
  try
    with Clients do
    try
