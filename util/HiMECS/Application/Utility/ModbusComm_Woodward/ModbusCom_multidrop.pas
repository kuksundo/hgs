...
uses
  ...
  Dialogs, CPort, DeCAL, IPCThrd_ECS_Woodward, IPCThrdClient_ECS_Woodward, ModbusComConst, CommonUtil,
  ...
  EngineParameterClass, IPC_ECS_Woodward_Const;
...
type
  ...
  private
    ...
    FIPCClient: TIPCClient_ECS_Woodward;//공유 메모리 및 이벤트 객체
    FIPCClient2: TIPCClient_ECS_Woodward;//공유 메모리 및 이벤트 객체
...
begin
  ...
  FIPCClient := TIPCClient_ECS_Woodward.Create(0, FSharedMemName, True);
  FIPCClient2 := TIPCClient_ECS_Woodward.Create(0, IPCCLIENTNAME2, True);
...
var
  ...
  EventData: TEventData_ECS_Woodward;
...
begin
  ...
  begin
    ...
    LStr := LStr + ''', Port = ' + IntToStr(FPortNum) + '), ' + IntToStr(SizeOf(EventData)) + ' ***'+#13#10;
    DisplayMessage(LStr, False);
  end//if
  else
  begin
