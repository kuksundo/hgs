...
uses classes, sysutils, Math, system.AnsiStrings;
...
type
  ...
  public
    ...
    Function ByteArrayToHexStr : String;
...
begin
  ...
  begin
    LStr := format('%.2x',[FBuffer[Index + i]]);
    Result := Result + LStr;
  end;
end;
...
function TByteArray2.ByteArrayToHexStr: String;
var I, L : Integer;
Begin
  Result := '';
  L := Length(FBuffer);
  ...
  Result := IntToHex(FBuffer[0],2);

  For I := 0 to L - 1 do
    Result := Result + IntToHex(FBuffer[I],2);
end;
