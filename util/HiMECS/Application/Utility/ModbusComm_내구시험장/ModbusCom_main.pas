unit ModbusCom_main;
{1. DSiGetProcessWindow함수를 이용하여 Window Handle을 가져올때 Main Form Handle이 아닌 문제 발생:
    (WM_CLOSE 메세지에 반응을 하지 않음)
   해결: Project Source에
        Application.MainFormOnTaskbar := True;
        추가하여 해결 함.                     : 2015.2.28
 2. TrayIcon을 추가한 후 Visible := True가 되면 위 내용이 무력화 되나 최초 실행시
    TrayIcon을 Visible := False로 두면 Main Form Handle을 가져올 수 있으므로 문제 없음
    .
}
interface
...
uses
  ...
  CopyData, ModBusTCPWagoThread, MBT2, JvExComCtrls,
  ...
  DeCAL, IPCThreadEvent, ModbusComConst_endurance, CommonUtil, HiMECSConst,
  IPCThrdClient_Generic, UnitIPCClientAll,
  IPCThrdMonitor_Generic, IPC_ModbusComm_Const, JclDebug, ModbusCom_Recv,
  UnitNetworkAdaptor, USock, Vcl.ImgList, Vcl.AppEvnts, TimerPool, SynCommons,
...
begin
  ...
  //LoadConfigDataini2Var함수 보다 먼저 실행 되어야 함.
...
begin
  ...
  begin
    ...
    begin
      if LCount = 1 then
      begin
        FParamSource := FEngineParameter.EngineParameterCollect.Items[i-1].ParameterSource; //SharedName2ParameterSource(FEngineParameter.EngineParameterCollect.Items[i-1].SharedName);
...
var
  ...
  i: integer;
begin
  //FSharedMemName = 'ModBusCom_kumo') or (FSharedMemName = 'KRAL') then
  ...
  iAddr := Str2Hex_Int(StartAddr) - FBaseAddress + i;//$5000;
...
*)
    ...
    begin
      SendData2MQ(LStr);
      DisplayMessage('MQ Send: ' + FMQServerIP + ', Port: ' + IntToStr(FMQServerPortNum) + ', Topic: ' + FMQTopic + ' >>> ' + LStr, False);
    end;
...
//  SetLength(EventData.ModBusAddress, 5);
  ...
  begin
    ...
    begin
      //공유메모리에 값을 저장함
      //제어기 Register가 16bit이고 수신된 데이터는 8bit이므로 두개를 합침
...
var
  ...
  EventData: TEventData_Modbus_Standard_DynArr;//TEventData_HIC;
        ...
        begin
          ...
          begin
            ...
            if FMQPasswd = '' then
              FMQPasswd := 'pjh';
            if FMQTopic = '' then
              FMQTopic := '/topic/' + FEngineName;
            if FMQServerIp = '' then
              FMQServerIp := '10.14.21.117';

            if not Assigned(FpjhSTOMPClass) then
              FpjhSTOMPClass := TpjhSTOMPClass.CreateWithStr(FMQUser,FMQPasswd,FMQServerIp,FMQTopic,Self.Handle,False,False);
          end;
        end;
...
begin
  ...
  try
    ...
    begin
      ...
      FMQServerEnable := ReadBool(MODBUS_SECTION, 'MQ Server Enable', False);
      FMQServerIp := ReadString(MODBUS_SECTION, 'MQ Server IP Address', '10.14.21.117');
      FMQServerPortNum := ReadInteger(MODBUS_SECTION, 'MQ Server Port', 61613);
      FMQServerBindIPAddress := ReadString(MODBUS_SECTION, 'MQ Server Bind IP Address', '');
      FMQUser := ReadString(MODBUS_SECTION, 'MQ Server UserID', 'pjh');
      FMQPasswd := ReadString(MODBUS_SECTION, 'MQ Server Passwd', 'pjh');
      FMQTopic := '/topic/' + ReadString(MODBUS_SECTION, 'MQ Server Topic', '');
    end;//with
...
begin
  FpjhSTOMPClass.StompSendMsg(AMsg, FMQTopic);
