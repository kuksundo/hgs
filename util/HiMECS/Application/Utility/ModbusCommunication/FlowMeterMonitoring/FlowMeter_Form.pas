...
uses
  ...
  Dialogs, ComCtrls, ExtCtrls, SyncObjs, ShadowButton,
  ModbusComStruct, DeCAL, IPCThrd_Kral, IPCThrdMonitor_Kral, ModbusComConst,
  DB, DBTables, XBaloon, XBaloon_Util, janSQL,
  EngMonitorConfig, iniFiles, Menus, StdCtrls, Watch, iComponent,
  iVCLComponent, iCustomComponent, iLCDMatrix;

const
  ENGMONITOR_SECTION = 'Engine Monitor';
  INIFILENAME = '.\FlowMeterMonitor.ini';
...
type
  ...
  TForm1 = class(TForm)
    ...
    procedure OnSignal2(Sender: TIPCThread_Kral; Data: TEventData_Kral);
  ...
  private
    ...
    FIPCMonitor2: TIPCMonitor_Kral;//공유 메모리 및 이벤트 객체
...
procedure TForm1.OnSignal(Sender: TIPCThread_Kral; Data: TEventData_Kral);
var
  i,dcount: integer;
begin
...
procedure TForm1.OnSignal2(Sender: TIPCThread_Kral; Data: TEventData_Kral);
var
  i,dcount: integer;
begin
  if not FMonitorStart then
...
var
  v_hnd: hwnd;
begin
  WinExec(PAnsiChar('.\DataSavep.exe'), SW_MINIMIZE);
...
var
  ...
  cc_hnd : hwnd;
begin
  WinExec(PAnsiChar('.\ModBusComP_Kral.exe'), SW_SHOWNOACTIVATE);
  v_hnd := FindWindow(nil, 'MODBUS 통신 화면(Multi-Drop)');
  if v_hnd <> 0 then
