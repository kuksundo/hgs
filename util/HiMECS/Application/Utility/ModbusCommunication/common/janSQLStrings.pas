...
uses
  ...
  function PosStr(FindString, SourceString: string;
...
function PosStr(FindString, SourceString: string; StartPos: Integer): Integer;
{asm
        PUSH    ESI
        PUSH    EDI
        PUSH    EBX
...
begin
  delete(SourceString, 1, StartPos-1);
  Result := pos(FindString, upperCase(SourceString));
  If Result = 0 then exit;
  If (Length(SourceString) > 0) and (Length(FindString) > 0) then
