...
uses
  ...
  TCPConfig, TCPServerConst, ComCtrls, SyncObjs, CoolTrayIcon;
...
type
  ...
  TServerFrmMain = class(TForm)
    ...
    procedure WMMT210Data(var Msg: TMessage); message WM_MT210_DATA;
  ...
  public
    ...
    FUseTimer: Boolean;//타이머를 이용해서 데이타 전송
    FTimerInterval: integer;//타이머를 이용할 때 전송 주기
    FDataReceived: Boolean;//데이터를 공유메모리로부터 수신하면 True
    FFirstOnce: boolean;//프로그램 시작시 타이머에서 처음 한번만 실행됨
    FCriticalSection: TCriticalSection;

    FIPCMonitor: TIPCMonitor2;//공유 메모리 및 이벤트 객체
    FIPAddress: string;
    FMT210Data: TEventData2; //공유메모리로부터 받은 데이터 저장

    procedure DisplayMessage(msg: string; ADspNo: TDisplayTarget);
    procedure InitVar;
    procedure SendData2Clients;
  end;
...
uses MT210_TCPUtil, TCPServer_Util;
...
begin
  ...
  try
    ...
    begin
      if FUseTimer then
      begin
        if FDataReceived then
        begin
          SendData2Clients;
          FDataReceived := False;
        end;

        Timer1.Interval := FTimerInterval;
      end;
    end;
  finally
    Timer1.Enabled := True;
...
begin
  ...
  try
    ...
    begin
      PortEdit.Text := ReadString(TCPSERVER_SECTION, 'Port', '47114');
      SharedMMNameEdit.Text := ReadString(TCPSERVER_SECTION, 'Shared Memory Name', IPCCLIENTNAME1);
      IntervalEdit.Text := ReadString(TCPSERVER_SECTION, 'Timer Interval', '1000');
      UseTimerCB.Checked := ReadBool(TCPSERVER_SECTION, 'Use Timer', False);
      UseTimerCBClick(self);
    end;//with
  finally
    iniFile.Free;
    iniFile := nil;
...
begin
  ...
  try
    ...
    begin
      ...
      FTimerInterval := ReadInteger(TCPSERVER_SECTION, 'Timer Interval', 1000);
      FUseTimer := ReadBool(TCPSERVER_SECTION, 'Use Timer', False);
    end;//with
  finally
    iniFile.Free;
    iniFile := nil;
...
begin
  ...
  try
    ...
    begin
      ...
      WriteBool(TCPSERVER_SECTION, 'Use Timer', UseTimerCB.Checked);
      WriteString(TCPSERVER_SECTION, 'Timer Interval', IntervalEdit.Text);
    end;//with
  finally
    iniFile.Free;
    iniFile := nil;
...
procedure TServerFrmMain.SendData2Clients;
var
  i: Integer;
  RecClient: PClient;
...
begin
  ...
  try
    ...
    begin
      ...
      RecThread.Connection.WriteBuffer(FMT210Data,SizeOf(FMT210Data), True);  // send the stuff
...
begin
  ...
  System.Move(Data, FMT210Data, SizeOf(TEventData2));
  SendMessage(Handle, WM_MT210_DATA, 0,0);
end;

//Client에게  MEXA-7000p에서 받은 데이터를 TCP/IP로 전달하는 함수///////////////
procedure TServerFrmMain.WMMT210Data(var Msg: TMessage);
begin
  if FUseTimer then
  begin
    FDataReceived := True;
