...
uses
  ...
  DeCAL, IPCThrd_MEXA7000, IPCThrdMonitor_MEXA7000,//janSQL,
  Menus, iProgressComponent, iLedBar,
  iPositionComponent, iScaleComponent, iGaugeComponent, iAngularGauge,
  ConfigConst, Options, Watchonfig, CircularArray;

type
  TWatchF2_ = class(TForm)
  ...
  private
    ...
    procedure UpdateTraceData(var Msg: TEventData_MEXA7000); message WM_EVENT_DATA;
  public
    FOwnerHandle: THandle;//Owner form handle
    FOwnerListIndex: integer;//TList에 저장되는 Index(해제시에 필요함)
    FIPCMonitor: TIPCMonitor_MEXA7000;//공유 메모리 및 이벤트 객체
...
begin
  ...
  FWatchCA := TCircularArray.Create(FAvgSize);
end;

procedure TWatchF2_.LoadConfigDataini2Form(ConfigForm: TWatchConfigF);
var
  iniFile: TIniFile;
begin
...
procedure TWatchF2_.LoadConfigDataini2Var;
var
  iniFile: TIniFile;
begin
  SetCurrentDir(FFilePath);
...
procedure TWatchF2_.SaveConfigDataForm2ini(ConfigForm: TWatchConfigF);
var
  iniFile: TIniFile;
begin
  SetCurrentDir(FFilePath);
...
procedure TWatchF2_.SetConfigData;
...
procedure TWatchF2_.Timer1Timer(Sender: TObject);
begin
  ...
  try
    ...
    begin
      ...
      FIPCMonitor := TIPCMonitor_MEXA7000.Create(0, FSharedName, True);
...
procedure TWatchF2_.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  FMonitorStart := False;
  Action := caFree;
...
procedure TWatchF2_.OnSignal(Sender: TIPCThread_MEXA7000; Data: TEventData_MEXA7000);
var
  i,dcount: integer;
begin
...
procedure TWatchF2_.UpdateTraceData(var Msg: TEventData_MEXA7000);
var
  i, j: integer;
  tmpStr: string;
...
procedure TWatchF2_.Displayalldatainthischart1Click(Sender: TObject);
var
  Li : integer;
begin
...
procedure TWatchF2_.DisplayMessage(Msg: string);
var
  i: integer;
begin
...
procedure TWatchF2_.WatchValue2Screen_Analog(Name: string; AValue: string);
var
  tmpdouble: double;
  tmpValue: string;
begin
  ...
  case PageControl1.ActivePageIndex of
    0: begin //simple
      ...
      AvgLabel2.Caption := format('%.2f',[FWatchCA.Average]);
    end;
    1: begin //Min/Max
...
procedure TWatchF2_.FormDestroy(Sender: TObject);
begin
  FWatchValueAry := nil;
...
procedure TWatchF2_.FormCreate(Sender: TObject);
begin
  InitVar;
end;

procedure TWatchF2_.Config1Click(Sender: TObject);
begin
  SetConfigData;
end;

procedure TWatchF2_.CurrentValue1Click(Sender: TObject);
begin
...
procedure TWatchF2_.Average1Click(Sender: TObject);
begin
...
procedure TWatchF2_.MinValue1Click(Sender: TObject);
begin
...
procedure TWatchF2_.MaxValue1Click(Sender: TObject);
begin
...
procedure TWatchF2_.Button2Click(Sender: TObject);
begin
  ApplyAvgSize;
end;

procedure TWatchF2_.ApplyAvgSize;
begin
  ...
  try
    ...
    FCurrentAryIndex := 0;
    FFirstCalcAry := True;
    FWatchCA.Size := FAvgSize;
  finally
    FCriticalSection.Leave;
  end;//try
end;

procedure TWatchF2_.FormShow(Sender: TObject);
begin
...
procedure TWatchF2_.Button1Click(Sender: TObject);
begin
  FWatchValueMax := 0;
  FWatchValueMin := 0;
end;
