...
uses
  ...
  JvComponentBase, JvgXMLSerializer, SBPro, JvExComCtrls, JvListView, Generics.Collections,
  HoribaComponentClasses, ConfigConst,
  MyKernelObject, CopyData, iComponent, iVCLComponent,//IPCThrd_MEXA7000, IPCThrdClient_MEXA7000,
  iCustomComponent, iAnalogDisplay, CLabels, MEXA7000_Watch_r1,
  UnitIPCClientAll, IPC_MEXA7000_Const, HiMECSConst, HiMECS_Watch2, EngineParameterClass;
...
//    FIPCClient: TIPCClient_MEXA7000;//공유 메모리 및 이벤트 객체
    ...
    procedure CreateWatch2Form(ALabel: string; ATag: integer);
  end;
...
begin
  CreateWatch2Form('CH4', ch4lbl.Tag);
end;
...
begin
  CreateWatch2Form('CO2', co2lbl.Tag);
end;

procedure TForm1.collblDblClick(Sender: TObject);
begin
  CreateWatch2Form('CO_L', collbl.Tag);
end;

procedure TForm1.CollectedlblDblClick(Sender: TObject);
begin
  CreateWatch2Form('Collected Value', Collectedlbl.Tag);
end;
...
procedure TForm1.CreateWatch2Form(ALabel: string; ATag: integer);
var
  LWatchF: TWatchF2;
  LIndex: integer;
  LEPIR: TEngineParameterItemRecord;
begin
  LWatchF := TWatchF2.Create(self);
  LWatchF.FOwnerHandle := Self.Handle;
  LWatchF.FOwnerListIndex := FWatchFormList.Add(LWatchF.Handle);
  LWatchF.FIsAutoFree := True;
//  LIndex := LWatchF.IPCMonitorAll1.FEngineParameter.GetItemIndex(ALabel);
  LIndex := FEngineParameter.GetItemIndex(ALabel);

  if LIndex <> -1 then
  begin
    FEngineParameter.EngineParameterCollect.Items[LIndex].AssignTo(LEPIR);
    LEPIR.FParamDragCopyMode := dcmCopyOnlyNonExist;
    LWatchF.FWG.FPM.SendEPCopyData(Handle, LWatchF.Handle, LEPIR);
  end;
...
begin
  for Li := FWatchFormList.Count - 1 Downto 0 do
    SendMessage(FWatchFormList.Items[Li], WM_CLOSE, 0, 0);
//    SendMessage(FWatchFormList.Items[Li], WM_SYSCOMMAND, SC_CLOSE, 0);
...
begin
  ...
  if FOptions.ParamFileName = '' then
  begin
//    FOptions.ParamFileName := ExtractFileName(Application.ExeName);
//    FOptions.ParamFileName := '..\doc\mexa7000\' + ChangeFileExt(FOptions.ParamFileName, '.param');
    FOptions.ParamFileName := '..\doc\mexa7000\mexa7000.param';
  end;

//  FIPCClient := TIPCClient_MEXA7000.Create(0, IPCCLIENTNAME1, True);
  FEngineParameter := TEngineParameter.Create(nil);
  FEngineParameter.LoadFromJSONFile(FOptions.ParamFileName);

  FIPCClientAll := TIPCClientAll.Create;
  FIPCClientAll.CreateIPCClient(psMEXA7000,IPCCLIENTNAME1);
  ...
  FWatchFormList := TList<integer>.Create;
...
begin
  CreateWatch2Form('THC', thclbl.Tag);
end;
...
begin
  ...
  SBPro.Panels[5].Text := IPCCLIENTNAME1;
end;
...
var
  EventData: TEventData_MEXA7000_2;
  LStr: string;
  Li: integer;
begin
  with EventData do
  try
    ...
    if Li > -1  then
    begin
      LStr := FOptions.Option.Items[Li].Value;
      co2lbl.Value := StrToFloatDef(LStr, 0.0);
      CO2 := co2lbl.Value;
    end;
    ...
    begin
      LStr := FOptions.Option.Items[Li].Value;
      collbl.Value := StrToFloatDef(LStr, 0.0);
      CO_L := collbl.Value;
    end;
    ...
    begin
      LStr := FOptions.Option.Items[Li].Value;
      o2lbl.Value := StrToFloatDef(LStr, 0.0);
    ...
    begin
      LStr := FOptions.Option.Items[Li].Value;
      NOxlbl.Value := StrToFloatDef(LStr, 0.0);
    ...
    begin
      LStr := FOptions.Option.Items[Li].Value;
      THClbl.Value := StrToFloatDef(LStr, 0.0);
      THC := THClbl.Value;
    end;
    ...
    begin
      LStr := FOptions.Option.Items[Li].Value;
      CH4lbl.Value := StrToFloatDef(LStr, 0.0);
      CH4 := CH4lbl.Value;
    end;
    ...
    begin
      LStr := FOptions.Option.Items[Li].Value;
      nonCH4lbl.Value := StrToFloatDef(LStr, 0.0);
      non_CH4 := nonCH4lbl.Value;
    end;
...
//  FIPCClient.PulseMonitor(EventData);
  FIPCClientAll.PulseEventData_MEXA7000(EventData);
end;
...
begin
  CreateWatch2Form('non-CH4', nonch4lbl.Tag);
end;

procedure TForm1.noxlblDblClick(Sender: TObject);
begin
  CreateWatch2Form('NOx', noxlbl.Tag);
end;

procedure TForm1.o2lblDblClick(Sender: TObject);
begin
  CreateWatch2Form('O2', o2lbl.Tag);
end;
...
begin
  ...
  begin
    ...
    try
      ...
      //begin
        Msg := Form1.IdTCPClient1.IOHandler.ReadLn;//('', 5);
        if Msg <> '' then
        begin
...
begin
  ...
  begin
    ...
    begin
      ...
      Sleep(FQueryInterval);
      Form1.IdTCPClient1.IOHandler.WriteLnRFC(FSendCommandListOnce.Strings[i]);
  ...
  begin
    ...
    begin
      ...
      //begin
        SendCopyData2(Form1.Handle, ' ', 1);
        Sleep(FQueryInterval);
        Form1.IdTCPClient1.IOHandler.WriteLnRFC(FSendCommandListRepeat.Strings[i]);
