...
uses
  ...
  DataSave2FileThread,DataSave2DBThread, TimerPool, ALed, ZConnection,
...
type
  TForm1 = class(TForm)
    ...
    lb_status: TLabel;
    Label1: TLabel;
    cb_LoadCellID: TComboBox;
    Label11: TLabel;
    cb_ExcelLog: TCheckBox;
    cb_DBLog: TCheckBox;
    Label3: TLabel;
    ed_InterSec: TEdit;
    Label10: TLabel;
...
//    procedure WMSaveData(var Msg: TWMModbusData); message WM_SAVEDATA;
    ...
    procedure bt_LogStartClick(Sender: TObject);
    procedure bt_LogEndClick(Sender: TObject);
...
//  if(now() < FDataSave2DBThread.LastLogTime + StrToTime('00:00:01')) then
//    exit;
  FCriticalSection.Enter;
  try
  tmpint := pHiMap.FValue;
  tmpdouble := tmpint;
  FSaveDataBuf := FloatToStr(tmpdouble/10);
  if(cb_DBLog.Checked = True) then
  begin
    SaveData2DB;
  end;
    FSaveDataBuf := DateTimeToStr(now)+FSeperator+FSaveDataBuf;
  if(cb_ExcelLog.Checked = True) then
  begin
    SaveData2File;
  end;
  finally
  ...
  StatusBar1.SimpleText := '마지막 데이터 : ' + FSaveDataBuf;
end;
...
begin
  ...
  begin
    FLoadCellID := cb_LoadCellID.Text;
...
procedure TForm1.bt_LogStartClick(Sender: TObject);
begin
  if(cb_LoadCellID.Text = '로드셀 선택') then
  begin
  ...
  FDataSave2DBThread.LastLogTime := Now;
  FLogStart := True;

  if(cb_ExcelLog.Checked = True) then
  begin
    FSaveDataBuf := #13#10 + 'LoadCell : ' + cb_LoadCellID.Text + Fseperator + 'Logging Start';
    SaveData2File;
  end;
  ...
  cb_LoadCellID.Enabled := False;
  ed_LogHour.Enabled := False;
  ed_LogMin.Enabled := False;
  ed_LogSec.Enabled := False;
  ed_InterMin.Enabled := False;
  ed_InterSec.Enabled := False;
  cb_ExcelLog.Enabled := False;
  cb_DBLog.Enabled := False;
end;

procedure TForm1.bt_LogEndClick(Sender: TObject);
begin
  ...
  cb_LoadCellID.Enabled := True;
  ed_LogHour.Enabled := True;
  ed_LogMin.Enabled := True;
  ed_LogSec.Enabled := True;
  ed_LogMin.Enabled := True;
  ed_LogSec.Enabled := True;
  cb_ExcelLog.Enabled := True;
  cb_DBLog.Enabled := True;
  ed_InterMin.Enabled := True;
  ed_InterSec.Enabled := True;  
