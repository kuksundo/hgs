...
uses Windows, classes, Forms, CPort, ModbusComConst, MyKernelObject, CopyData, SysUtils;
...
Type
  TModBusComThread = class(TThread)
    ...
    LastLogTime: TDateTime;//마지막 로그 기록 시간(통신 복구용)
...
begin
  ...
  begin
    if(now() > LastLogTime+StrToTime('00:00:01')) then
    begin
...
begin
  ...
  try
    ...
    begin
      ...
      //0A0D가 없으면 아직 완성되지 않은 패킷임
      if System.Pos(#$D#$A, FBufStr) = 0 then
        exit;
