...
{$WARNINGS OFF}
{$ENDIF}
		constructor Create(list : TThreadList);  // intentional hiding of base class method
{$IFDEF WARNINGSON}
{$WARNINGS ON}
{$ENDIF}
	end;
...
{ SpecialTypes } //Modify by pjh at 2002.7.15
var
{$IFDEF USELONGWORD}
	specialTypes : DeCALBasicTypes = [vtInterface, vtString, vtAnsiString, vtCurrency, vtVariant, vtWideString, vtExtended, vtInt64];
{$ELSE}
	specialTypes : DeCALBasicTypes = [vtInterface, vtString, vtAnsiString, vtCurrency, vtVariant, vtWideString, vtExtended];
{$ENDIF}
...
{ clearDObject } //modify by pjh at 2002.7.15(patch)
procedure ClearDObject(var obj : DObject);
begin
	if (obj.VType in SpecialTypes) then
		begin
			case obj.VType of
        vtInterface:
          IUnknown(obj.VInterface)._Release;
				vtString:
					FreeMem(obj.VString);
				vtAnsiString:
					begin
...
{ copyDObject }//Modify by pjh at 2002.7.15
procedure CopyDObject(const source : DObject; var dest : DObject);
begin
	...
	if (source.VType in SpecialTypes) then
        ...
        vtInterface:
				  begin
            dest.VInterface := source.VInterface;
            IUnknown(dest.VInterface)._AddRef;
          end; //vtInterface
				vtString:
					begin
						New(dest.VString);
...
begin
	iter := startKey;
  result := DeCAL_pjh.countIn(iter, finish, key);
end;

function DInternalHash._countValues(const value : DObject) : Integer;
begin
  result := DeCAL_pjh.count(self, value);
end;
...
constructor DTList.Create(list : TThreadList);
begin
end;
