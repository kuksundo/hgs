...
uses
  ...
  Dialogs,janSQLstrings,janSQLExpression2,janSQLTokenizer, mwStringHashList,
  Variants;
...
type
  ...
  private
    ...
    Fcatalog: string;
    FFileExt: string;//add by pjh 2003.9.24 .txt 뿐만 아니라 다른 확장자도 지원
  ...
  public
    { Public declarations }
    constructor create(TokenSeperator: string = ';');
    destructor  destroy; override;
...
// split atext at g_TokenSeperator into lines
...
begin
  ...
  p1:=1;
  repeat
    p2:=PosStr(g_TokenSeperator,atext,p1);
    if p2>0 then begin
...
procedure split2(atext:string;alist:TStrings);
var
  tmp:string;
  p1,p2,L:integer;
begin
...
constructor TjanSQL.create(TokenSeperator: string = ';');
begin
  DecimalSeparator:='.';
  g_TokenSeperator := TokenSeperator;
  FFileExt := '.txt';
...
{new code}
  ...
  split2(sqloutput.FieldNames, recordsets[t3].FieldNames);  //2
...
begin
  ...
  for i:=1 to c do
    if (recordsets[i].persistent) and (recordsets[i].modified) then begin
      fn:=Fcatalog+'\'+recordsets[i].name+FileExt;
      if not fileexists(fn) then exit;
...
var
  ...
  begin
    for ii:=0 to fc-1 do begin
      ...
      tosqlMin:
        begin
          //if forcefloat(s1)<forcefloat(s2) then  //by pjh 2003.03.02
          if s1 < s2 then
          arecordset.records[torecord].fields[ii].value:=s1;
        end;
      tosqlsum:
...
begin
  ...
  fn:=Fcatalog+'\'+recordsets[idx+1].name+FileExt;
...
procedure TjanSQL.SetFileExt(AValue: String);
begin
