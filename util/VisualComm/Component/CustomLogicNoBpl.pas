unit CustomLogicNoBpl;

interface

uses
  vcl.ExtCtrls, Windows, Messages, SysUtils, Classes, vcl.Graphics, vcl.Controls, vcl.Forms,
  vcl.Dialogs, math, GR32, GR32_Polygons, TimerPool, ScrollPanel2;
...
type
  ...
  //TCustomLogicPanel = class(TPanel)
  //TCustomLogicPanel = class(TpjhScrollPanel)
  ...
  protected
    ...
    function GetCheckDrawPanel: TCustomLogicPanel;
    property CheckDrawPanel: TCustomLogicPanel read GetCheckDrawPanel;
    procedure Paint; override;
  ...
  public
    ...
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    property DrawPanel: TCustomLogicPanel read FDrawPanel;
  ...
  private
    ...
    FBeforeDelay,
    FAfterDelay: Longint;
    FOwner: TpjhScrollPanel;//TWincontrol;
  ...
  public
    { Public declarations }
    FIsInitialize: Bool; //Logic이 Reset 되었을 때 Node를 초기화 하기 위함  
...
begin
  ...
  //ReadDFM 시에 에러 발생 - "Class TpjhTimerPool not found"
  //Create function 에서 parameter 를 Self 가 아닌 nil로 주니 해결됨
...
begin
  //Parent가 Form일 경우에는 자동 실행 되므로 Free를 안해도 됨
  //FTimerPool.Free;
...
begin
  ...
  if (AComponent is TCustomLogicControl) and (TCustomLogicControl(AComponent).DrawPanel = self) then
    case Operation of
      opRemove:
        begin
          Invalidate;
...
begin
  if (Value <> nil) and (Value.FDrawPanel <> self) then
...
begin
  ...
  with Source.DrawPanel.Canvas do
  begin
    Pen.Style := psSolid;
  //Pen.Color := clBlack;
    ...
    spLR,spRR,spTR,spBR:
      begin
        ...
        Lines[dmDestination].x :=
          Lines[dmDestination].x-Source.DrawPanel.Canvas.Pen.Width*Direction;
    ...
    spLT,spRT,spTT,spBT:
      begin
        ...
        Lines[dmDestination].y :=
          Lines[dmDestination].y-Source.DrawPanel.Canvas.Pen.Width*Direction;
...

{
  ...
  Source.DrawPanel.Canvas.Pen.Handle := ExtCreatePen(
    PS_GEOMETRIC or PS_SOLID or PS_ENDCAP_FLAT or PS_JOIN_ROUND, // PS_JOIN_BEVEL,
    Source.DrawPanel.Canvas.Pen.Width,
  ...
  if (csDesigning in Source.DrawPanel.ComponentState) then
  begin
    if (Selected) and (Source.DrawPanel.FConnector = self) then
    begin
      for i := dmFirstHandle to dmLastHandle do
      begin
        //디자인 타임에 마우스로 화살표 선택시에 3개의 점을 찍어줌
        Source.DrawPanel.Canvas.Rectangle(Lines[i].x-2, Lines[i].y-2,
                                                  Lines[i].x+2, Lines[i].y+2);
      end;

      //화살표 양끝점에 연두색 점을 그려줌
      Source.DrawPanel.Canvas.Brush.Color := clLime;
      i := dmSource;
      Source.DrawPanel.Canvas.Ellipse(Lines[i].X-4, Lines[i].Y-4,
...
begin
  ...
  //if (AParent <> nil) and not (AParent.InheritsFrom(TCustomLogicPanel)) then
  begin
    //ShowMessage(BoolToStr(AParent.ClassType.InheritsFrom(TCustomLogicPanel)));
  ...
  begin
    //ShowMessage(BoolToStr(AParent.ClassType.InheritsFrom(TCustomLogicPanel)));
    FDrawPanel := TCustomLogicPanel(AParent);
  end;
end;
...
begin
  //if (FDrawPanel <> nil) then
  //  FDrawPanel.ChangeState(self);
end;

function TCustomLogicControl.GetCheckDrawPanel: TCustomLogicPanel;
begin
  if (FDrawPanel = nil) then
...
begin
  if not(Value) then
    CheckDrawPanel.State := nil
  else
    CheckDrawPanel.State := self;
end;

function TCustomLogicControl.GetActive: boolean;
begin
  Result := (CheckDrawPanel.State = self);
end;
...
begin
  PrepareCanvas(veConnector, DrawPanel.Canvas);
end;
...
begin
  inherited Notification(AComponent, Operation);
  if (Operation = opRemove) and (AComponent = self) and (FDrawPanel <> nil) then
...
begin
  ...
  //if (DrawPanel <> nil) and (DrawPanel.State = nil) then
  //  DrawPanel.State := self;
end;
...
begin
  ...
  if Assigned(FDefaultTransition) then
    DrawPanel.ChangeState(FDefaultTransition)
  else
    Result := False;
end;
...
begin
  ...
  FOwner := TpjhScrollPanel(AOwner);//TForm(AOwner);
  FDefault := True;
...
begin
  ...
  if (DefaultState) then
    ...
    DrawPanel.Canvas.Pen.Width := 1;
  DrawPanel.Canvas.Pen.Color := clGreen;
  ...
  if not(DefaultState) then
    ...
    DrawPanel.Canvas.Pen.Width := 1;
  DrawPanel.Canvas.Pen.Color := clRed;
...
//  LBmp.Canvas.CopyRect(LBmp.BoundsRect, Canvas, r);
  ...
  rgn0 := CreatePolygonRgn(Diamond,8, WINDING);
  FillRgn(Canvas.Handle, rgn0, Canvas.Brush.Handle);
  FrameRgn(Canvas.Handle, rgn0, Canvas.Pen.Handle,Canvas.Pen.Width, Canvas.Pen.Width);
  DeleteObject(rgn0);
...
begin
  ...
  begin
    FDefault := Value;
    DrawPanel.Invalidate; // To erase previous fat line
    // Invalidate;
  end;
end;
...
begin
  ...
  begin
    ...
    if not(DrawPanel.StateChanged) then
    begin
      if (FResult) and (Assigned(FTrueState)) then
        DrawPanel.ChangeState(FTrueState)
      else if (not FResult) and (Assigned(FFalseState)) then
        DrawPanel.ChangeState(FFalseState);
    end;
  end;
...
begin
  if (Value <>  nil) and (Value.DrawPanel <> CheckDrawPanel) then
...
begin
  ...
  if Assigned(NextStep) then
    DrawPanel.ChangeState(NextStep)
  else
    Result := False;
end;
