...
//      <------- dx ----->
  ...
  //point(x,y)가 r1 사각형의 어디에 위치하는지 판단함
  //
  ...
  begin
    ...
    if y > r1.Y then code := [Bottom]
    else if y < r1.Y then code := [Top];

    if x > r1.X then code := code + [Right]
    else if x < r1.X then code := Code + [Left];
  end;
begin
...
{  if (dx = 0) then
  ...
  case ActualPath of
    spLL, spLR, spLT, spLB:
      begin
        CompOutCode(Lines[dmFirstHandle].x, Lines[dmFirstHandle].y, OutCode);
        //도착점이 시작점의 오른쪽에 있기 때문에 두번째점은 위 또는 아래로 가야함
        if right in OutCode then
        begin
          //도착점이 시작점의 위쪽에 있음
          if top in OutCode then
            //위아래로 겹쳤고 밑부분에 연결된 경우에만 해당됨
            if (OverLapY) and (ActualPath = spLB) then
          ...
          //도착점이 시작점의 아래쪽에 있음
          if Bottom in OutCode then
            //위아래로 겹쳤고 윗부분에 연결된 경우
            if (OverLapY) and (ActualPath = spLT) then
              Lines[dmSecondHandle] := Point(Lines[dmFirsthandle].X, r1.Top - DefaultMargin)
            else
              Lines[dmSecondHandle] := Point(Lines[dmFirstHandle].X, max(r1.Bottom + DefaultMargin, Lines[dmLastHandle].Y));
        end
        else
        //도착점이 시작점의 왼쪽에 있기 때문에 두번째점은 좌로 가야함
        if left in OutCode then
        begin
          //도착점이 시작점의 위쪽에 있음
          if top in OutCode then
          begin
            if (OverLapX) then
              if not(OverLapY) and (ActualPath = spLR) then
                Lines[dmSecondHandle] := Point(Lines[dmFirstHandle].x,
                                                       r1.Top - DefaultMargin)
              else
                Lines[dmSecondHandle] := Point(min(Lines[dmFirstHandle].x,
                              r2.Left - DefaultMargin), Lines[dmFirstHandle].y)
            else
