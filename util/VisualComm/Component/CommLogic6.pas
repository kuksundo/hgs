...
begin
  Result := False;
  for i := dmFirstHandle to dmOffset do
  begin
...
var
  ...
  DirectionX		,
  DirectionY		: Shortint;
  tmpPoint: array[0..3,0..3] of TPoint;
  tmpConnector1: array[0..3] of integer;
  tmpConnector2: array[0..3] of integer;
  i,j,k: integer;
  L1,L2: TLine_pjh;
...
{  if (dx = 0) then
  ...
  tmpConnector1[0] := Lines[dmFirstHandle].X;
  tmpConnector1[1] := Lines[dmLastHandle].X;
  tmpConnector1[2] := r1.Right + (d2x div 2);
  tmpConnector1[3] := r1.Left - (d2x div 2);

  tmpConnector2[0] := Lines[dmFirstHandle].y;
  tmpConnector2[1] := Lines[dmLastHandle].y;
  tmpConnector2[2] := r1.Top - (d2y div 2);
  tmpConnector2[3] := r1.Bottom + (d2y div 2);

  for i := 0 to 3 do
    for j := 0 to 3 do
      tmpPoint[i][j] := Point(tmpConnector1[i],tmpConnector2[j]);

  for i:= 0 to 3 do
  begin
    for j := 0 to 3 do
    begin
      if (Lines[dmFirstHandle].x = tmpPoint[i][j].x) or
                                (Lines[dmFirstHandle].y = tmpPoint[i][j].y) then
      begin
        ...
        L2.p2 := tmpPoint[i][j];
        //두개의 라인이 교차 되었다면
        if LineInterSect(L1, L2, LPointOnLine) then
          if not LPointOnLine then //Cross상태라면 건너뜀
            Continue;

        Lines[dmSecondHandle] := tmpPoint[i][j];

        for k := 0 to 3 do
        begin
          if k <> j then
          begin
            if ((Lines[dmLastHandle].X = tmpPoint[i][k].X) or
              (Lines[dmLastHandle].Y = tmpPoint[i][k].Y)) then
            begin
              L1.p1 := Lines[dmFirstHandle];
              L1.p2 := Lines[dmSecondHandle];
              L2.p1 := Lines[dmSecondHandle];
              L2.p2 := tmpPoint[i][k];
              //두개의 라인이 교차 되었다면
              if LineInterSect(L1, L2, LPointOnLine) then
                if not LPointOnLine then //Cross상태라면 건너뜀
              ...
              Lines[dmThirdHandle] := tmpPoint[i][k];
        ...
        begin
          if k <> i then
          begin
            if ((Lines[dmLastHandle].X = tmpPoint[k][j].X) or
              (Lines[dmLastHandle].Y = tmpPoint[k][j].Y)) then
            begin
              L1.p1 := Lines[dmFirstHandle];
              L1.p2 := Lines[dmSecondHandle];
              L2.p1 := Lines[dmSecondHandle];
              ...
              Lines[dmThirdHandle] := tmpPoint[k][j];
              Lines[dmOffset] := Lines[dmThirdHandle];
