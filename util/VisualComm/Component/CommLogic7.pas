...
type
  ...
  TLineType = (ltOne, ltTwo, ltThree, ltFour, ltFive);
  ...
  protected
    ...
    function GetLineType(dx,dy: integer; ox,oy: Boolean): TLineType;
  public
...
function TCustomLogicConnector.GetLineType(dx,dy: integer; ox,oy: Boolean): TLineType;
begin
  if dx > 0 then
  begin
    if dy > 0 then//(+,+)
    ...
    spLL, spLR, spLT, spLB:
      begin
        if not OverLapY then
          ...
          Lines[dmSecondHandle] := Point(Lines[dmFirstHandle].x - DefaultMargin, p1.y);
      end;
    spRL, spRR, spRT, spRB:
      if not OverLapX and (DirectionX > 0) then
        Lines[dmSecondHandle] := Point(Lines[dmSource].x + d2x DIV 2, p1.y)
      else
        Lines[dmSecondHandle] := Point(Lines[dmSource].x + DefaultMargin, p1.y);
    spTL, spTR, spTT, spTB:
      if not OverLapY and (DirectionY < 0) then
        Lines[dmSecondHandle] := Point(p1.x, Lines[dmSource].y - d2y DIV 2)
      else
        Lines[dmSecondHandle] := Point(p1.x, Lines[dmSource].y - DefaultMargin);
    spBL, spBR, spBT, spBB:
      if not OverLapY and (DirectionY > 0) then
        Lines[dmSecondHandle] := Point(p1.x, Lines[dmSource].y + d2y DIV 2)
      else
