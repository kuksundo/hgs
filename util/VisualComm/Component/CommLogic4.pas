...
type
  ...
  TDesignMove = (dmSource, dmFirstHandle, dmSecondHandle, dmThirdHandle, dmOffset, dmLastHandle, dmDestination, dmNone);
  ...
  case ActualPath of
    spLL, spRL, spTL, spBL:
      begin
        ...
        //Lines[dmLastHandle] := Point(Lines[dmDestination].x - d2x DIV 2, p2.y);
    ...
    spLT, spRT, spTT, spBT:
      begin
        ...
        //Lines[dmLastHandle] := Point(p2.x, Lines[dmDestination].y - d2y DIV 2);
    ...
    begin
      ...
      begin
        ...
        begin
          if OverLapY then
          begin
            Lines[dmSecondHandle] := Point(Lines[dmFirstHandle].x, r1.Top - DefaultMargin);
            Lines[dmThirdHandle] := Point(Lines[dmLastHandle].x, Lines[dmSecondHandle].y);
          end
          else
          begin
            Lines[dmSecondHandle] := Point(Lines[dmFirstHandle].x, Lines[dmDestination].y);
            Lines[dmThirdHandle] := Point(Lines[dmFirstHandle].x, Lines[dmDestination].y);
          end;

          Lines[dmOffset] := Lines[dmThirdHandle];
        end
        else //(-,-)
        begin
          if OverLapX then
          begin
            //Lines[dmSecondHandle] := Point(Lines[dmFirstHandle].x, Lines[dmDestination].y);
            //Lines[dmThirdHandle] := Point(Lines[dmSecondHandle].x, Lines[dmSecondHandle].y);
            Lines[dmSecondHandle] :=
              Point(Min(Lines[dmLastHandle].x,Lines[dmFirstHandle].x), Lines[dmFirstHandle].y);
            Lines[dmThirdHandle] := Point(Lines[dmSecondHandle].x, Lines[dmLastHandle].y);
          end
          else
          if OverLapY then
          begin
            Lines[dmSecondHandle] :=
              Point(Lines[dmFirstHandle].x, Min(r1.Top,r2.Top) - DefaultMargin);
            Lines[dmThirdHandle] := Point(Lines[dmLastHandle].x, Lines[dmSecondHandle].y);
          end
          else
          begin
            Lines[dmSecondHandle] := Point(Lines[dmLastHandle].x, Lines[dmSource].y);
            Lines[dmThirdHandle] := Point(Lines[dmLastHandle].x, Lines[dmSource].y);
          end;
          Lines[dmOffset] := Point(Lines[dmThirdHandle].x, Lines[dmThirdHandle].y);
        end;
      end
      else
      if DirectionY > 0 then
      begin
        ...
        begin
          ...
          begin
            Lines[dmSecondHandle] :=
                      Point(Min(Lines[dmLastHandle].x,Lines[dmFirstHandle].x),
                      r1.Bottom + DefaultMargin);
            Lines[dmThirdHandle] := Point(Lines[dmLastHandle].x, Lines[dmSecondHandle].y);
          end
          else
          begin
            Lines[dmSecondHandle] := Point(Lines[dmFirstHandle].x, Lines[dmDestination].y);
            Lines[dmThirdHandle] := Point(Lines[dmSecondHandle].x, Lines[dmDestination].y);
          end;
          Lines[dmOffset] := Point(Lines[dmThirdHandle].x, Lines[dmDestination].y);
        end
        else//(-,+)
        begin
          //y방향으로 overlap될때 변경해야 함
          if OverLapY then
          begin
            if OverLapX then
            begin
              Lines[dmSecondHandle] := Point(Lines[dmLastHandle].x, Lines[dmFirstHandle].y);
              Lines[dmThirdHandle] := Point(Lines[dmSecondHandle].x, Lines[dmLastHandle].y);
            end
            else
            begin
              Lines[dmSecondHandle] := Point(Lines[dmFirstHandle].x,
                                    Max(r1.Bottom,r2.Bottom) + DefaultMargin);
              Lines[dmThirdHandle] := Point(Lines[dmLastHandle].x, Lines[dmSecondHandle].y);
            end;
          end
          else
          ...
          //Lines[dmSecondHandle] := Point(Lines[dmLastHandle].x, p1.y);
          //Lines[dmThirdHandle] := Point(Lines[dmLastHandle].x, p1.y);
          Lines[dmOffset] := Point(Lines[dmThirdHandle].x, Lines[dmThirdHandle].y);
        end;
      end//else if
      else//DirectionY = 0
      begin
        Lines[dmSecondHandle] := Point(Lines[dmFirstHandle].x, Lines[dmSource].y + Source.Height);
        Lines[dmThirdHandle] := Point(Lines[dmLastHandle].x, Lines[dmSource].y + Source.Height);
        Lines[dmOffset] := Point(Lines[dmLastHandle].x, Lines[dmSource].y + Source.Height);
      end;
    end;//spLL
  spLR:
    begin
      ...
      begin
        if DirectionX > 0 then //(+,-)
        begin
          Lines[dmSecondHandle] := Point(Lines[dmFirstHandle].x, min(r1.Top + DefaultMargin, Lines[dmLastHandle].Y));
          Lines[dmThirdHandle] := Point(Lines[dmLastHandle].x, Lines[dmSecondHandle].y);//=spLL
          Lines[dmOffset] := Lines[dmThirdHandle];
        end
        else //(-,-)
        begin
          ...
          begin
            ...
            Lines[dmSecondHandle] := Point(Lines[dmFirstHandle].x, Lines[dmLastHandle].y);
            Lines[dmThirdHandle] := Point(Lines[dmSecondHandle].x, Lines[dmSecondHandle].y);
          end;
          Lines[dmOffset] := Point(Lines[dmThirdHandle].x, Lines[dmThirdHandle].y);
        end;
      ...
      begin
        ...
        begin
          if OverLapX then
          begin
            Lines[dmSecondHandle] := Point(Lines[dmFirstHandle].x, r1.Bottom + DefaultMargin);
            Lines[dmThirdHandle] := Point(Lines[dmLastHandle].x, Lines[dmSecondHandle].y);
          end
          else
          if OverLapY then
  ...
  begin
    if (Selected) and (Source.StateMachine.FConnector = self) then
      ...
      begin
        ...
        if (i <> dmOffset) or not(ActualPath in [spTL, spRB]) then
