...
type
  TtaskHome_Frm = class(TForm)
    ...
    procedure NxNumberEdit1Change(Sender: TObject);
...
procedure TtaskHome_Frm.NxNumberEdit1Change(Sender: TObject);
begin
...
begin
  with DM1.OraQuery1 do
  begin
    Label5.Caption := Label5.Caption +' ('+FormatDateTime('yyyy-MM',today)+'월)';
    for li := 1 to 3 do
    begin
      Close;
      SQL.Clear;
      SQL.Add('SELECT SUM(RST_MH) MH FROM TMS_RESULT A, TMS_RESULT_MH B ' +
              'WHERE A.RST_NO = B.RST_NO ' +
              'AND RST_PERFORM >= :begin  ' +
              'AND RST_PERFORM <= :end ' +
              'AND RST_BY IN (SELECT USERID FROM HITEMS_USER WHERE DEPT_CD LIKE :TEAM )');

      ParamByName('begin').AsDate := StartOfTheMonth(Today);
      ParamByName('end').AsDate := EndOfTheMonth(Today);
      ParamByName('TEAM').AsString := 'K2B'+IntToStr(li)+'%';
      Open;

      case li of
        1 : NxNumberEdit1.Value := FieldByName('MH').AsFloat;
        2 : NxNumberEdit2.Value := FieldByName('MH').AsFloat;
        3 : NxNumberEdit3.Value := FieldByName('MH').AsFloat;
      end;
    end;
  end;
end;
