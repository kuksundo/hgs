  ...
  begin
    ...
    begin
      ...
      begin
        Close;
        SQL.Clear;
        SQL.Add('SELECT * FROM TMS_TASK ' + 'WHERE TASK_NO = :param1 ');
        ...
        begin
          ...
          begin
            ...
            SQL.Add('SELECT * FROM ' + '( ' + '   SELECT ' +
              ...
              + '       ) A LEFT JOIN TMS_PLAN B ' +
              '       ON A.PN = B.PLAN_NO ' +
              '       AND A.PRN = B.PLAN_REV_NO ' + '   )  ' +
              '   WHERE TASK_NO IN ' + '   ( ' +
              '       SELECT A.TASK_NO FROM TMS_TASK A, TMS_TASK_SHARE B ' +
  ...
  begin
    ...
    try
      ...
      begin
        Close;
        SQL.Clear;
        SQL.Add('SELECT ENGMODEL FROM HIMSEN_INFO ' + 'AND ENGTYPE = :ENGTYPE '
  ...
  begin
    ...
    try
      ...
      begin
        Close;
        SQL.Clear;
        SQL.Add('SELECT * FROM HITEMS_DEPT ' + 'WHERE PARENT_CD = :param1 ' +
          'ORDER BY DEPT_CD ');
      ...
      begin
        ...
        begin
          ...
          begin
            Close;
            SQL.Clear;
            SQL.Add('SELECT * FROM TMS_AUTHORITY ' + 'WHERE USERID = :param1 ');
  ...
  TThread.Queue(nil,
    ...
    const
      Query = 'SELECT * FROM ' + '(         ' + '   SELECT ' +
        ...
        + '   FROM TMS_TASK A, TMS_TASK_SHARE B ' + '   WHERE  ' + '   ( ' +
        '     ( ' +
        '       TO_CHAR(EXD_TASK_START, ''yyyy-mm-dd'') <= :beginDate  ' +
        '       AND EXD_TASK_END >= TO_DATE(:beginDate, ''yyyy-mm-dd'')  ' +
        '       AND TO_CHAR(EXD_TASK_START, ''yyyy-mm-dd'') <= :endDate  ' +
...
const
  Query = 'SELECT * FROM ' + '( ' + '   SELECT * FROM ' + '   ( ' +
    ...
    '               FROM TMS_PLAN A, HITEMS_CODE_GROUP B ' +
...
//        DeleteRow(ARow);
        ...
        try
          ...
          begin
            Close;
            SQL.Clear;
            SQL.Add('SELECT * FROM TMS_TASK ' + 'START WITH TASK_NO = :param1 '
              + 'CONNECT BY PRIOR TASK_NO = TASK_PRT ' +
              'ORDER SIBLINGS BY TASK_ORDER ');
            ParamByName('param1').AsString := lResult;
            Open;
    ...
    try
      ...
      begin
        ...
        begin
          Close;
          SQL.Clear;
          SQL.Add('UPDATE TMS_TASK SET ' + 'TASK_ORDER = :TASK_ORDER ' +
            'WHERE TASK_NO = :param1 ');
...
begin
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('SELECT A.*,B.*, (SELECT COUNT(*) FROM TMS_TEST_RECEIVE_INFO WHERE PLAN_NO = A.PLAN_NO) TESTCNT  ' +
            ' FROM TMS_PLAN A ' +
            '                INNER JOIN   (  ' +
            '                SELECT GRP_NO, CODE_NAME FROM HITEMS_CODE_GROUP   ) B  ' +
