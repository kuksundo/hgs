...
uses
  ...
  AdvGlowButton, AdvPanel, Vcl.Imaging.jpeg, Vcl.ExtCtrls, StrUtils, Ora;
    ...
    begin
      ...
      begin
        ...
        SQL.Add('SELECT DEPT_NAME FROM HITEMS.HITEMS_DEPT ' +
                'WHERE DEPT_CD = (SELECT DEPT_CD FROM HITEMS.HITEMS_USER ' +
                'where USERID = :param1 )');
  ...
  begin
    ...
    try
      ...
      begin
        Close;
        SQL.Clear;
        SQL.Add('SELECT A.*, B.DESCR FROM HITEMS.HITEMS_USER A, HITEMS.HITEMS_USER_GRADE B ' +
                'WHERE GUNMU = :param1 ' +
                'AND A.GRADE = B.GRADE ' +
                'AND SUBSTR(DEPT_CD,1,3) = (SELECT DEPT_CD FROM HITEMS.HiTEMS_DEPT ' +
...
function TfindUser_Frm.Get_DeptName(aDeptCd: String): String;
var
  OraQuery : TOraQuery;
begin
  ...
  try
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Add('SELECT * from HITEMS.HITEMS_DEPT where DEPT_CD = :param1 ');
    ...
    try
      ...
      begin
        ...
        begin
          Close;
          SQL.Clear;
          SQL.Add('SELECT DEPT_CD FROM HITEMS.HITEMS_DEPT ' +
                  'where DEPT_NAME = :param1 ');
...
//                  'AND DEPT_NAME = :param1 '+
                  'ORDER BY DEPT_CD ');
//          ParamByName('param1').AsString := Get_DeptName(CurrentUsersDept);
          Open;
    ...
    try
      ...
      begin
        ...
        SQL.Add('SELECT A.*, B.DESCR FROM HITEMS.HiTEMS_USER A, HITEMS.HITEMS_USER_GRADE B ' +
                'WHERE GUNMU = :param1 '+
                'AND A.GRADE = B.GRADE '+
                'AND DEPT_CD like ''%''||(SELECT DEPT_CD from HITEMS.HiTEMS_DEPT WHERE DEPT_NAME = :param2 )||''%'' ');
