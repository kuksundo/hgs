...
uses
  ...
  Vcl.Graphics, System.Types, WinApi.Windows, system.DateUtils;

type
  TTempClass = class(TObject)
...
//    begin
//      TpjhPlannerDatePicker(AComponent.Components[i]).Calendar.OnCellDraw := TTempClass.PlannerCalendarCellDraw;
//    end;
//  end;
//end;
...
class procedure TTempClass.PlannerCalendarCellDraw(Sender: TObject; Canvas: TCanvas;
  Day: TDate; Selected, Marked, InMonth: Boolean; Rect: TRect);
var
  da,mo,ye: word;
  s: string;
...
class procedure TTempClass.SetPlannerCalDellDraw(AComponent: TComponent);
var
  i: integer;
begin
  for i := 0 to AComponent.ComponentCount - 1 do
...
class procedure TTempClass.SetPlannerCalEvent(AComponent: TComponent);
var
  i,j,k: integer;
