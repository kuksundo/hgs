...
type
  TchooseMS_Frm = class(TForm)
    ...
    Panel5: TPanel;
    Image1: TImage;
    MSNumberRB: TRadioButton;
    MSNameRB: TRadioButton;
    ...
    begin
      ...
      begin
        ...
        begin
          Close;
          SQL.Clear;
          SQL.Add('SELECT MS_NO, NVL(PARENT_NO,1) PARENT_NO, UPPER(MS_NAME) MS_NAME FROM HIMSEN_MS_NUMBER ' +
  ...
  begin
    ...
    try
      ...
      begin
        ...
        begin
          if MSNumberRB.Checked then
            filter := Cell[0, i].AsString
          else
          if MSNameRB.Checked then
            filter := Cell[2, i].AsString;

          rv := (s = '') or (Pos(s, UpperCase(filter)) > 0);
          RowVisible[i] := rv;

          if MSNumberRB.Checked then
          begin
            if s <> '' then
              Cell[0,i].TextColor := clRed
            else
              Cell[0,i].TextColor := clBlack;
