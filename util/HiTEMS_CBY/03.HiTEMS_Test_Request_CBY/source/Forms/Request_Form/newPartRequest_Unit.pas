...
type
  ...
  public
    ...
    procedure Get_Choose_List(aTestPartItem:TTestPartItem);
    procedure Init_Info;
...
procedure TnewPartRequest_Frm.Button1Click(Sender: TObject);
var
  LChooseMSHistoryF: TChooseMSHistoryF;
  LReqNo: string;
begin
  if et_msNumber.Text = '' then
  begin
    ShowMessage('부품명을 먼저 선택 하세요!');
    exit;
  end;

  if grid_choose.RowCount > 0 then
  begin
    LReqNo := '이미 선택한 부품이 존재 합니다. OK 버튼을 누르면 기존의 부품(들)이 삭제됩니다.' +#13+#10 +
              '그래도 계속 하시겠습니까?';
    if MessageDlg(LReqNo, mtConfirmation , mbOKCancel , 0) = mrCancel then
    begin
      exit;
    end;
  end;
  ...
  LChooseMSHistoryF := TChooseMSHistoryF.Create(nil);

  try
    with LChooseMSHistoryF do
    begin
      FormStyle := fsNormal;
      Position  := poScreenCenter;

      et_msNumber.Text := self.et_msNumber.Text;
      et_msNumber.Hint := self.et_msNumber.Hint;
      cb_engType.Text := self.EngTypeLbl.Caption;

      if ShowModal = mrOK then
      begin
        LReqNo := grid_Req.CellByName['tc_reqNo', grid_Req.SelectedRow].AsString;
        Get_Request_Resource(LReqNo);
      end;
    end;
  finally
    LChooseMSHistoryF.Free;
  end;

//  (EngTypeLbl.Caption, et_msNumber.Hint, et_msNumber.Text);
end;
...
procedure TnewPartRequest_Frm.Get_Choose_List(aTestPartItem:TTestPartItem);
var
  i : Integer;
begin
  with grid_choose do
  begin
    ...
    try
      ...
      begin
        ...
        ParamByName('param1').AsString := aTestPartItem.PART_NO;
        Open;

        if RecordCount <> 0 then
        begin
          i := aTestPartItem.RowNo;
...
procedure TnewPartRequest_Frm.Get_Request_Resource(aReqNo: String);
var
  i : Integer;
