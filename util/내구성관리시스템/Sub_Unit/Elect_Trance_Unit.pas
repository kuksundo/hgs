...
type
  ...
  public
    { Public declarations }
    procedure Eelec_Trance_the_edit(aEquipno: string);
  end;

var
  Eelec_Trance_Frm: TEelec_Trance_Frm;
  function Create_Eelec_Trance_Frm(aEquipNo:String):Boolean;
...
begin
  Result := False;
  Eelec_Trance_frm := TEelec_Trance_frm.Create(nil);
  try
    with Eelec_Trance_frm do
    begin
      if aEquipNo <> '' then
      begin
        FEquipNo := aEquipNo;
        Eelec_Trance_the_edit(FEquipNo);
      end;
    ...
    FreeAndNil(Eelec_Trance_frm);
  end;
end;

{ TControlPanel_Frm }

procedure TEelec_Trance_Frm.Button1Click(Sender: TObject);
begin
  Close;
end;

procedure TEelec_Trance_Frm.Eelec_Trance_the_edit(aEquipno: string);
var
  str : String;
  ms : TMemoryStream;
  bmp : TPNGImage;
...
procedure TEelec_Trance_Frm.Pn_SpecClick(Sender: TObject);
var
  LHint : String;
  //aEquipno: string;
begin
