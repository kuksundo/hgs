...
type
  TnewTmsCategory_Frm = class(TForm)
    Panel8: TPanel;
    Image1: TImage;
    lb_title: TJvLabel;
    JvLabel3: TJvLabel;
...
var
  newTmsCategory_Frm: TnewTmsCategory_Frm;
  function Create_newTmsCategory_Frm(aParentNo,aCatNo:String;aCat_LV:Integer):Boolean;
...
begin
  Result := False;
  newTmsCategory_Frm := TnewTmsCategory_Frm.Create(nil);
  try
    with newTmsCategory_Frm do
    begin
      if aCatNo <> '' then
      begin
    ...
    FreeAndNil(newTmsCategory_Frm);
  end;
end;

{ TnewCategory_Frm }

procedure TnewTmsCategory_Frm.AeroButton1Click(Sender: TObject);
begin
...
procedure TnewTmsCategory_Frm.FormShow(Sender: TObject);
begin
  et_CatName.SetFocus;
end;

procedure TnewTmsCategory_Frm.Get_CategoryInfo(aCatNo: String);
var
  OraQuery : TOraQuery;
begin
...
function TnewTmsCategory_Frm.Get_CategoryName(aCatNo: String): String;
var
  OraQuery : TOraQuery;
begin
  OraQuery := TOraQuery.Create(nil);
...
procedure TnewTmsCategory_Frm.Insert_NewCategory;
var
  LCat_No : String;
begin
...
procedure TnewTmsCategory_Frm.Update_Category(aCatNo: String);
begin
  with DM1.OraQuery1 do
  begin
    Close;
