    ...
    try
      ...
      begin
        ...
        SQL.Add('SELECT  ' +
                ...
                '   SELECT ' +
                '     TEST_ENGINE, ' +
                '     (SELECT ENGTYPE FROM DPMS_HIMSEN_INFO WHERE PROJNO LIKE TEST_ENGINE) ENGTYPE ' +
                '   FROM DPMS_TMS_TEST_REQUEST A ' +
  ...
  begin
    ...
    try
      ...
      begin
        ...
        SQL.Add('SELECT ' +
                ' REQ_DEPT, ' +
                ' (SELECT DEPT_NAME FROM DPMS_DEPT WHERE DEPT_CD LIKE REQ_DEPT) DEPT_NAME ' +
                'FROM DPMS_TMS_TEST_REQUEST A ' +
                'GROUP BY REQ_DEPT ' +
                'ORDER BY REQ_DEPT ');
    ...
    try
      ...
      begin
        ...
        SQL.Add('SELECT * FROM ' +
                ...
                '     REQ_ID, ' +
                '     (SELECT DEPT_NAME FROM DPMS_DEPT WHERE DEPT_CD LIKE REQ_DEPT) DEPT_NAME, ' +
                '     (SELECT NAME_KOR FROM DPMS_USER WHERE USERID LIKE REQ_ID) REQ_ID_NAME ' +
                '   FROM DPMS_TMS_TEST_REQUEST A ' +
    ...
    try
      ...
      begin
        ...
        SQL.Add('SELECT * FROM   ' +
                ...
                '   SELECT   ' +
                '     A.*,   ' +
                '     (SELECT ENGTYPE FROM DPMS_HIMSEN_INFO WHERE PROJNO LIKE TEST_ENGINE) ENG_TYPE, ' +
                '     (SELECT NAME_KOR FROM DPMS_USER WHERE USERID LIKE REQ_ID) USER_NAME,   ' +
                '     (SELECT DEPT_NAME FROM DPMS_DEPT WHERE REQ_DEPT LIKE DEPT_CD) DEPT_NAME  ' +
                '   FROM   ' +
                '   (   ' +
                '     SELECT A.*, B.PLAN_NO, NVL(B.STATUS,''미접수'') STATUS, RECEIVE_ID ' +
                '     FROM DPMS_TMS_TEST_REQUEST A LEFT OUTER JOIN DPMS_TMS_TEST_RECEIVE_INFO '+
...
//  lhead := '123456780123456789012';
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('SELECT REQ_ID FROM DPMS_TMS_TEST_REQUEST ' +
...
begin
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('UPDATE DPMS_TMS_TEST_RECEIVE_INFO SET ' + 'STATUS = ''완료'' ' +
      'WHERE REQ_NO LIKE :param1 ');
