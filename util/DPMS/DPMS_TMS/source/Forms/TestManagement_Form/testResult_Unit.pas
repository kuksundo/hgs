...
begin
  ...
  begin
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Add('SELECT * FROM DPMS_TMS_TEST_RECEIVE_INFO ' +
...
begin
  ...
  begin
    ...
    SQL.Add('SELECT ' +
            '  A.*, ' +
            '  (SELECT ENGTYPE FROM DPMS_HIMSEN_INFO WHERE PROJNO LIKE A.LDS_ENGPROJ) ENG_TYPE, ' +
            '  (SELECT RPM FROM DPMS_HIMSEN_INFO WHERE PROJNO LIKE A.LDS_ENGPROJ) ENG_CYLNUM, ' +
            '  (SELECT BORE FROM DPMS_HIMSEN_INFO WHERE PROJNO LIKE A.LDS_ENGPROJ) ENG_BORE, ' +
            '  (SELECT STROKE FROM DPMS_HIMSEN_INFO WHERE PROJNO LIKE A.LDS_ENGPROJ) ENG_STROKE, ' +
            '  (SELECT RPM FROM DPMS_HIMSEN_INFO WHERE PROJNO LIKE A.LDS_ENGPROJ) ENG_RPM, ' +
            '  B.* ' +
            'FROM DPMS_TMS_DATA_LOCAL A, DPMS_TMS_DATA_LOCAL_VALUE B ' +
            'WHERE A.LDS_NO = B.LDS_NO ' +
            'AND ORDER_NO IN ( ' +
            '   SELECT ORDER_NO FROM DPMS_TMS_WORK_ORDERS ' +
            '   WHERE PLAN_NO LIKE ( ' +
            '       SELECT PLAN_NO FROM DPMS_TMS_TEST_RECEIVE_INFO ' +
            '       WHERE REQ_NO LIKE :param1 ' +
            '   )' +
            ') ' +
            'ORDER BY LDS_DATE ');
    ...
    begin
      ...
      begin
        MyResourceStream := TResourceStream.Create(hInstance, 'Resource_2', RT_RCDATA);
        try
    ...
    try
      ...
      begin
        ...
        SQL.Add('SELECT  ' +
                ...
                ' RST_BY, ' +
                ' RST_MH, ' +
                ' (SELECT NAME_KOR FROM DPMS_USER WHERE USERID LIKE RST_BY) RST_BY_NAME ' +
...
begin
  ...
  begin
    ...
    SQL.Add('SELECT REMARK FROM ' +
            '( ' +
            '   SELECT ORDER_NO, MAX(SEQ_NO) SEQ_NO FROM DPMS_TMS_WORK_RESULT ' +
            '   WHERE ORDER_NO LIKE :PARAM1 ' +
            '   GROUP BY ORDER_NO ' +
            ') A LEFT OUTER JOIN ' +
            '( ' +
            '   SELECT * FROM DPMS_TMS_WORK_RESULT ' +
            ') B ' +
...
begin
  ...
  begin
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Add('SELECT PERFORM FROM DPMS_TMS_WORK_ORDERS ' +
              'WHERE PLAN_NO IN ( ' +
              '   SELECT PLAN_NO FROM DPMS_TMS_TEST_RECEIVE_INFO ' +
              '   WHERE REQ_NO LIKE :param1 ' +
              ') ' +
    ...
    try
      ...
      begin
        ...
        SQL.Add('SELECT * FROM ' +
                ...
                '       ( ' +
                '         SELECT ORDER_NO FROM DPMS_HIMSEN_MS_MOUNTED_PART UNION ' +
                '         SELECT ORDER_NO FROM DPMS_TMS_DATA_LOCAL UNION ' +
                '         SELECT ORDER_NO FROM DPMS_TMS_DATA_SHIM ' +
                '       ) WHERE ORDER_NO LIKE A.ORDER_NO     ' +
                '     ) RESULT_CNT, ' +
                '     ( ' +
                '       SELECT CODE_NAME FROM ' +
                '       ( ' +
                '         SELECT CAT_NO CODE, CAT_NAME CODE_NAME FROM DPMS_TMS_WORK_CATEGORY UNION ' +
                '         SELECT GRP_NO CODE, CODE_NAME FROM DPMS_TMS_WORK_CODEGRP  ' +
                ...
                '       SELECT  ' +
                '         A.*, ' +
                '         (SELECT NAME_KOR FROM DPMS_USER WHERE USERID LIKE WORKERID) NAMEK ' +
                '       FROM ' +
                '       ( ' +
                '         SELECT  ' +
                '           ORDER_NO,  ' +
                ...
                '         FROM ' +
                '         ( ' +
                '           SELECT ORDER_NO, STATUS, LIST_OF_WORKERS FROM DPMS_TMS_WORK_RESULT ' +
                '           WHERE ORDER_NO IN ( ' +
                '             SELECT ORDER_NO FROM DPMS_TMS_WORK_ORDERS ' +
...
begin
  ...
  with grid_buss do
  begin
    Preview_BussLog(Cells[3,ARow],  //업무수행일
                    Cells[1,ARow],  //결과번호
                    Cells[6,ARow]); //담당자 아이디
...
procedure TtestResult_Frm.mi_msfileOpenClick(Sender: TObject);
var
  lms: TMemoryStream;
  litem : TListItem;
  lFileName : String;
...
begin
  ...
  begin
    ...
    begin
      ...
      ParamByName('param1').AsString := fileGrid.Cells[1,fileGrid.SelectedRow];
  ...
  try
    ...
    begin
      ...
      SQL.Add('SELECT  ' +
              ...
              'FROM ' +
              '( ' +
              '   SELECT * FROM DPMS_TMS_WORK_CODEGRP WHERE GRP_NO LIKE :CODE ' +
              ') A LEFT OUTER JOIN ' +
              '( ' +
