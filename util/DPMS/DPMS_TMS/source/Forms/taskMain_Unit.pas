...
type
  TtaskMain_Frm = class(TForm)
    ...
    TaskPlanBtn: TAdvGlowButton;
    TaskResultBtn: TAdvGlowButton;
    ...
    procedure TaskPlanBtnClick(Sender: TObject);
    procedure TaskResultBtnClick(Sender: TObject);
  ...
  private
    ...
    procedure SetAlarm;
    procedure UpdateSetAlarm;
    procedure SendSMS_Alarm(AIndex: integer);
  public
    procedure SetUI;
  end;
...
procedure TtaskMain_Frm.TaskPlanBtnClick(Sender: TObject);
begin
  ISCreateForm(TtaskManagement_Frm,'taskManagement_Frm','[업무관리]');
end;

procedure TtaskMain_Frm.TaskResultBtnClick(Sender: TObject);
var
  lstr,
  luserId : String;
begin
...
procedure TtaskMain_Frm.UpdateSetAlarm;
var
  i: integer;
begin
  i := DM1.FToDoCollect4Alarm.FCurrentAlarmIndex;
  //ToDo_List Table의 AlarmFalg를 Update함
  DM1.UpdateAlarmFlag;
  SetAlarm;
  SendSMS_Alarm(i);
end;
...
procedure TtaskMain_Frm.AdvNavBar1Change(Sender: TObject);
begin
//  ShowMessage(IntToStr(AdvNavBar1.ActiveTabIndex));
end;

procedure TtaskMain_Frm.AlarmFromTo1AlarmBegin(Sender: TObject);
begin
  UpdateSetAlarm;
end;
...
begin
  ...
  begin
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Add('SELECT * FROM DPMS_APP_VERSION ' +
...
begin
  ...
  DM1.GetToDoListFromDB2(DM1.FToDoCollect4Alarm);
...
procedure TtaskMain_Frm.SendSMS_Alarm(AIndex: integer);
var
  LReqID,
  lhead,
  ltitle,
...
begin
  lhead    := 'DPMS ';
  ltitle   := '미리알림 ';
  LReqID := DM1.FUserInfo.UserID;

  LMsg := '일정:' + DM1.FToDoCollect4Alarm.Items[AIndex].Subject + #13#10 +
          '시작시간: '+FormatDateTime('yyyy-MM-dd hh:mm', DM1.FToDoCollect4Alarm.Items[AIndex].CreationDate);
...
procedure TtaskMain_Frm.SetAlarm;
var
  i: integer;
  Ldt, Ldt2: TDateTime;
...
begin
  ...
  begin
    if Ldt > DM1.FToDoCollect4Alarm.Items[i].AlarmTime  then
      continue;

    if DM1.FToDoCollect4Alarm.Items[i].AlarmFlag = 1 then
      continue;

    Ldt := Date;
    DecodeDateTime(DM1.FToDoCollect4Alarm.Items[i].AlarmTime, myYear, myMonth, myDay, myHour, myMin, mySec, myMilli);
    Ldt2 := EncodeDate(myYear, myMonth, myDay);

    //오늘 날짜 이면
    if Ldt = Ldt2 then
    begin
      //AlarmTime 순으로 Sort 되어 있음(AScending)
      AlarmFromTo1.AlarmTimeBegin := FormatDateTime('hh:mm:ss',DM1.FToDoCollect4Alarm.Items[i].AlarmTime);
      AlarmFromTo1.ActiveBegin := True;
      DM1.FToDoCollect4Alarm.FCurrentAlarmIndex := i;
      exit;
    end;
  end;
end;

procedure TtaskMain_Frm.SetUI;
begin
  CurvyPanel2.Height := TaskPlanBtn.Height + TaskPlanBtn.Margins.Top + TaskPlanBtn.Margins.Bottom +
                        TaskResultBtn.Height + TaskResultBtn.Margins.Top + TaskResultBtn.Margins.Bottom + 10;
//  AdvOfficeStatusBar1.PanelIndex[0]
  AdvOfficeStatusBar1.Panels.Items[0].Text := DM1.FUserInfo.UserName + ' ' + DM1.FUserInfo.Grade + '(' + DM1.FUserInfo.UserID + ')';
end;
