...
type
  TlocalSheet_Frm = class(TForm)
    ...
    procedure DATA91Change(Sender: TObject);
  ...
  public
    ...
    function GetEngineDataFromDB(AEngProjNo: string): Boolean;
...
begin
  ...
  begin
    ...
    begin
      ...
      if not GetEngineDataFromDB(aEngProjNo) then
        exit;
    end;
    Show;
  end;
...
procedure TlocalSheet_Frm.AeroButton1Click(Sender: TObject);
begin
  GetEngineDataFromDB(FEngProjNo);
end;
...
//    Close;
//    SQL.Clear;
//    SQL.Add('DELETE FROM TMS_DATA_LOCAL ' +
...
procedure TlocalSheet_Frm.DATA91Change(Sender: TObject);
begin
  if FEngMCR > 0 then
    et_load.AsInteger := Round((DATA91.Value / FEngMCR)*100);
end;
...
function TlocalSheet_Frm.GetEngineDataFromDB(AEngProjNo: string): Boolean;
var
  LDataTable : string;
begin
  Result := False;
  ...
  if LDataTable = '' then
  begin
    ShowMessage('엔진타입에 해당하는 DB Table이 존재하지 않습니다!');
    Exit;
  end;
...
begin
  ...
  try
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Add('SELECT ENGTYPE, MCR FROM DPMS_HIMSEN_INFO ' +
      ...
      if RecordCount <> 0 then
      begin
        FEngMCR := FieldByName('MCR').AsInteger;
...
begin
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('SELECT * FROM DPMS_TMS_DATA_LOCAL ' +
    ...
    begin
      ...
      Close;
      SQL.Clear;
      SQL.Add('SELECT B.* FROM DPMS_TMS_DATA_LOCAL A, DPMS_TMS_DATA_LOCAL_VALUE B ' +
...
begin
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('INSERT INTO DPMS_TMS_DATA_LOCAL ' +
    ...
    ParamByName('LDS_ID').AsString        := DM1.FUserInfo.CurrentUsers;
    ExecSQL;
    Result := True;
  end;
end;
...
begin
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('INSERT INTO DPMS_TMS_DATA_LOCAL_VALUE ');
    SQL.Add(LSQL);
    ExecSQL;
  end;
end;
