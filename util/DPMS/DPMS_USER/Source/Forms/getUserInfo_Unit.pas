...
begin
  ...
  begin
    ...
    begin
      ShowMessage('DPMS_DEPT_ALIAS Table에 데이터가 존재합니다.');
      exit;
    end;
    ...
    if RecordCount = 0 then
    begin
      ShowMessage('DPMS_DEPT Table에 데이터가 없습니다.');
      exit;
    end;
  end;
...
begin
  ...
  if (aPartName = '') and (aPosition = '부서장') then
  begin
    Result := aCode + '0';
    exit;
  end
  else
  if (aPartName = '') and ((aPosition = '담당임원') or (aPosition = '부문장') or
    (aPosition = '부본부장') or (aPosition = '사업대표')) then
  begin
    Result := aCode + 'K00';
    exit;
  end;
  ...
  try
    ...
    begin
      ...
      if cb_deptcode.Text = 'A' then
        ...
        SQL.Add('select * from kx01.gtaa004 ' +
              'where DEPTNM IS NOT NULL AND DEPT = :param1 AND STATNM = ''재직''' //''K%''  AND EMPNO = ''A379042''
        );
...
//부서별 DPMS_USER 테이블 갱신할 때  해당 부서원들을 먼저 GUNMU를 'O'로 설정 해야 함.
