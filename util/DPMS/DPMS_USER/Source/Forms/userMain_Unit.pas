...
uses
  ...
  NxDBColumns, Vcl.Grids, Vcl.DBGrids, GIFImg, Vcl.Menus, Generics.Collections;
...
type
  TdetailUser_Frm = class(TForm)
    ...
    N2: TMenuItem;
    N3: TMenuItem;
    CreateDPMSUSERGRADE1: TMenuItem;
    CreateDPMSDEPT1: TMenuItem;
    N4: TMenuItem;
    Button1: TButton;
    ...
    procedure CreateDPMSUSERGRADE1Click(Sender: TObject);
    procedure CreateDPMSDEPT1Click(Sender: TObject);
...
//    function ISCreateForm(aClass: TFormClass; aName ,aCaption : String): TForm;
    ...
    function Update_DPMS_USER : Boolean;
    ...
    procedure Update_DPMS_User_Priv(APriv, APosition: string);
    procedure Insert_DPMS_USER_GRADE_From_HiTEMS;
    procedure Insert_DPMS_DEPT_From_kx01_gtaa004;
    //DPMS_DEPT와 kx01_gtaa004의 부서코드를 비교하여 DMPS_DEPT의 부서명을 갱신함
    procedure Update_DPMS_DEPT_From_kx01_gtaa004;
    procedure Insert_DPMS_DEPT_TEAM_From_kx01_gtaa004;
    procedure Delete_DPMS_DEPT;
  end;
...
procedure TdetailUser_Frm.Button1Click(Sender: TObject);
begin
  Delete_DPMS_DEPT;
  Insert_DPMS_DEPT_TEAM_From_kx01_gtaa004;
end;
...
procedure TdetailUser_Frm.CreateDPMSDEPT1Click(Sender: TObject);
begin
  Insert_DPMS_DEPT_From_kx01_gtaa004;
end;

procedure TdetailUser_Frm.CreateDPMSUSERGRADE1Click(Sender: TObject);
begin
  Insert_DPMS_USER_GRADE_From_HiTEMS;
end;

procedure TdetailUser_Frm.Delete_DPMS_DEPT;
begin
  ...
  begin
    ...
    try
      ...
      begin
        Close;
        SQL.Clear;
        SQL.Add('SELECT A.GRADE, B.DESCR FROM DPMS_USER A, DPMS_USER_GRADE B  ' +
  ...
  begin
    ...
    try
      ...
      begin
        Close;
        SQL.Clear;
        SQL.Add('SELECT * FROM DPMS_USER A, DPMS_USER_GRADE B, DPMS_DEPT C ' +
                'WHERE A.GRADE = B.GRADE ' +
                'AND A.GUNMU = ''I'' ' +
...
procedure TdetailUser_Frm.Insert_DPMS_DEPT_From_kx01_gtaa004;
var
  LStr, LDept: string;
...
begin
  DM1.OraQuery1.Close;
  DM1.OraQuery1.SQL.Clear;
  DM1.OraQuery1.SQL.Add('Select count(*) reccnt from DPMS_DEPT');
  DM1.OraQuery1.Open;

  if DM1.OraQuery1.FieldByName('reccnt').AsInteger > 0 then
  begin
    ShowMessage('DPMS_DEPT Table is not empty!' + #13#10 + 'Not executed.');
    exit;
  end;
  ...
  begin
    ...
    SQL.Clear;
    //INSA DB에서 가져옴(엔진사업부)
    SQL.Add('SELECT DISTINCT DEPT, DEPTNM, PARTCD, PARTNM FROM kx01.gtaa004 ');
    SQL.Add('WHERE division = ''K'' and gubun = ''Z01'' and deptnm is not null ');
    SQL.Add('ORDER BY DEPT, PARTNM ');
    ...
    begin
      ...
      try
        DM1.OraQuery1.Close;
        DM1.OraQuery1.SQL.Clear;
        DM1.OraQuery1.SQL.Add('INSERT INTO DPMS_DEPT VALUES( :PARENT_CD,:DEPT_CD,:DEPT_NAME,:DEPT_LV, NULL,NULL,NULL,NULL )');

        LStrList := TStringList.Create;
        try
          while not eof do
          begin
            LStr := FieldByName('PARTCD').AsString;
            LDept := Copy(FieldByName('DEPT').AsString, 1,3);

            //팀코드가 null이면 DEPT 필드는 부서코드임
            if LStr = '' then
            begin
              DM1.OraQuery1.ParamByName('PARENT_CD').AsString := '';
              DM1.OraQuery1.ParamByName('DEPT_CD').AsString := LDept;
              DM1.OraQuery1.ParamByName('DEPT_NAME').AsString := FieldByName('DEPTNM').AsString;
              DM1.OraQuery1.ParamByName('DEPT_LV').AsInteger := 1;
            end
            else
            begin
              if LStrList.IndexOf(LDept) = -1 then
              begin
                LStrList.Add(LDept);
                i := 1;
              end;

              LStr := LDept + IntToStr(i);
              Inc(i);

              DM1.OraQuery1.ParamByName('PARENT_CD').AsString := LDept;
              DM1.OraQuery1.ParamByName('DEPT_CD').AsString := LStr;
              DM1.OraQuery1.ParamByName('DEPT_NAME').AsString := FieldByName('PARTNM').AsString;
              DM1.OraQuery1.ParamByName('DEPT_LV').AsInteger := 2;
            end;

            DM1.OraQuery1.ExecSQL;
...
procedure TdetailUser_Frm.Insert_DPMS_DEPT_TEAM_From_kx01_gtaa004;
var
  LPartCodeList: TStringList;
  LKey, LDeptCode, LPartCode, LDeptName: string;
  i: integer;
  LPartDic: TDictionary<string, TStringList>;
begin
  with DM1.OraQuery2 do
  begin
    Close;
    ...
    LPartDic := TDictionary<string, TStringList>.Create;
    try
      while not eof do
      begin
        //부서코드;부서이름
        LDeptCode := LeftStr(FieldByName('DEPT').AsString, 3) + ';' + FieldByName('DEPTNM').AsString;

        if not LPartDic.ContainsKey(LDeptCode) then
        begin
          LPartCodeList := TStringList.Create;
          LPartDic.Add(LDeptCode, LPartCodeList);
        end;

        LPartCode := FieldByName('PARTNM').AsString;

        if LPartCode <> '' then
          LPartDic.Items[LDeptCode].Add(LPartCode);
      ...
      DM1.OraTransaction1.StartTransaction;
      try
        for LKey in LPartDic.Keys do//부서코드;부서이름
        begin
          LDeptName := LKey;
          LDeptCode := StrToken(LDeptName,';');
          LPartCode := RightStr(LDeptName, 1);

          with DM1.OraQuery1 do
          begin
            Close;
            SQL.Clear; //부서 입력
            SQL.Add('INSERT INTO DPMS_DEPT (PARENT_CD, DEPT_CD, DEPT_NAME, DEPT_LV) ');
            SQL.Add('    VALUES(:PARENT_CD, :DEPT_CD, :DEPT_NAME, :DEPT_LV)');
            ParamByName('PARENT_CD').AsString := '';
            ParamByName('DEPT_CD').AsString := LDeptCode;
            ParamByName('DEPT_NAME').AsString := LDeptName;
            ...
            SQL.Add('INSERT INTO DPMS_DEPT (PARENT_CD, DEPT_CD, DEPT_NAME, DEPT_LV) ');
            SQL.Add('    VALUES(:PARENT_CD, :DEPT_CD, :DEPT_NAME, :DEPT_LV)');
            ParamByName('PARENT_CD').AsString := LDeptCode;
            ParamByName('DEPT_CD').AsString := LDeptCode + '0';

            if LPartCode = '부' then
              ParamByName('DEPT_NAME').AsString := LDeptName + '서장'
            else
            if LPartCode = '실' then
              ParamByName('DEPT_NAME').AsString := LDeptName + '장'
            else
              ParamByName('DEPT_NAME').AsString := LDeptName;
            ...
            //팀 입력
            for i := 0 to LPartDic.Items[LKey].Count - 1 do
            begin
              Close;
              SQL.Clear;
              SQL.Add('INSERT INTO DPMS_DEPT (PARENT_CD, DEPT_CD, DEPT_NAME, DEPT_LV) ');
              SQL.Add('    VALUES(:PARENT_CD, :DEPT_CD, :DEPT_NAME, :DEPT_LV)');
              ParamByName('PARENT_CD').AsString := LDeptCode;
              ParamByName('DEPT_CD').AsString := LDeptCode + IntToStr(i+1);
              ParamByName('DEPT_NAME').AsString := LPartDic.Items[LKey].Strings[i];
              ParamByName('DEPT_LV').AsInteger := 2;
              ExecSQL;
            end;//for
          end;//with
      ...
      for LDeptCode in LPartDic.Keys do
        LPartDic.Items[LDeptCode].Free;
...
procedure TdetailUser_Frm.Insert_DPMS_USER_GRADE_From_HiTEMS;
begin
  DM1.OraQuery1.Close;
  DM1.OraQuery1.SQL.Clear;
  DM1.OraQuery1.SQL.Add('Select count(*) reccnt from DPMS_USER_GRADE');
  DM1.OraQuery1.Open;
  ...
  begin
    ...
    begin
      ...
      try
        DM1.OraQuery1.Close;
        DM1.OraQuery1.SQL.Clear;
        DM1.OraQuery1.SQL.Add('INSERT INTO DPMS_USER_GRADE VALUES( :p1,:p2,:p3,:p4 )');

        while not eof do
        begin
          DM1.OraQuery1.ParamByName('p1').AsString := FieldByName('GRADE').AsString;
          DM1.OraQuery1.ParamByName('p2').AsString := FieldByName('DESCR').AsString;
          DM1.OraQuery1.ParamByName('p3').AsInteger := FieldByName('STDTIME').AsInteger;
          DM1.OraQuery1.ParamByName('p4').AsString := FieldByName('EOT').AsString;
          DM1.OraQuery1.ExecSQL;
...
begin
  ...
  try
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Add('SELECT * FROM TMS_PLAN_INCHARGE A, DPMS_USER B ' +
...
procedure TdetailUser_Frm.Update_DPMS_DEPT_From_kx01_gtaa004;
var
  LDeptCode, LDeptName: string;
begin
  with DM1.OraQuery2 do
  begin
    Close;
    ...
    DM1.OraTransaction2.StartTransaction;
    try
      while not eof do
      begin
        LDeptCode := LeftStr(FieldByName('DEPT').AsString, 3);
        LDeptName := FieldByName('DEPTNM').AsString;

        with DM1.OraQuery1 do
        begin
          Close;
          SQL.Clear;
          SQL.Add('UPDATE DPMS_DEPT SET ' +
                  'DEPT_NAME = :DEPT_NAME ' +
       ...
       DM1.OraTransaction2.Commit;
    except
      DM1.OraTransaction2.Rollback;
    end;
  end;
end;

function TdetailUser_Frm.Update_DPMS_USER: Boolean;
var
  ms : TMemoryStream;
  OraQuery : TOraQuery;
begin
...
procedure TdetailUser_Frm.Update_DPMS_User_Priv(APriv, APosition: string);
begin
  with DM1.OraQuery1 do
  begin
    Close;
        ...
        begin
          ...
          begin
            ...
            begin
              Close;
              SQL.Clear;
              SQL.Add('DELETE FROM DPMS_USER_ANNIVERSARY ' +
            ...
            begin
              Close;
              SQL.Clear;
              SQL.Add('INSERT INTO DPMS_USER_ANNIVERSARY ' +
                      'VALUES( ' +
