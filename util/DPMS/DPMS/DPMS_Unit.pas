...
type
  ...
  public
    ...
    function GetDPMSAppCode(AAppName: string): string;
  end;
...
begin
  ...
  try
    with HiTEMS_Frm do
    begin
      aHwnd := FindWindow(nil,'SPMS[Sales Process Management System]');
      if aHwnd > 0then
      begin
...
//    appCode := '20131119111223777';
    appCode := GetDPMSAppCode('DPMS_LOGIN.EXE');
...
function THiTEMS_Frm.GetDPMSAppCode(AAppName: string): string;
begin
  Result := '';
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('SELECT APPCODE FROM DPMS_APP_CODE ' +
            'WHERE APPNAME_E = :AppName');
    ParamByName('AppName').AsString := AAppName;
