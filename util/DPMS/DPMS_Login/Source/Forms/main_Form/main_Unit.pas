...
uses
  ...
  JvProgressDialog, Vcl.Menus, JvExComCtrls, JvComCtrls, AdvOfficeImage, HiMECSUserClass;
...
begin
  ...
  try
    with LHiMECSUser.HiMECSUserCollect.Add do
    begin
      UserID := CurrentUserId;
      Password := CurrentUserPwd;
    end;

    LHiMECSUser.SaveToJSONFile(AutoLoginFileName, ExtractFileName(AutoLoginFileName), True);
  finally
    LHiMECSUser.Free;
  end;
      ...
      begin
        ...
        begin
          ...
          SQL.Add('SELECT A.USERID, B.* FROM ' +
                  'DPMS_USER_APPLICATION A, DPMS_APP_CODE B ' +
          ...
          try
            ...
            begin
              ...
              SQL.Add('SELECT * FROM DPMS_APP_VERSION  ' +
                      'WHERE VERNO = ( ' +
                      '                 SELECT MAX(VERNO) FROM DPMS_APP_VERSION ' +
    ...
    begin
      ...
      begin
        ...
        begin
          Close;
          SQL.Clear;
          SQL.Add('SELECT APPCODE, LASTWRITETIME FROM DPMS_APP_VERSION ' +
                  'WHERE APPCODE = :param1 ' +
          ...
          begin
            if IsRunningProcess(appCode+'.exe') then
            begin
              ShowMessage('신규 버젼이 존재합니다.' + #13#10 + AppName + ' 프로그램이 실행 중입니다.' + #13#10 + '새로운 버젼으로 업데이트를 위해 프로세스('+ appCode+ '.exe)를 종료 하세요.');
              exit;
            end;
