...
uses
  ...
  JvRichEditToHtml, JvDialogs, WMCopyData, UnitClipBoardUtil;
...
type
  TEditorMainForm = class(TForm)
    ...
    SaveFromClipBrd1: TMenuItem;
    SavetoClipBrd1: TMenuItem;
    ...
    procedure SavetoClipBrd1Click(Sender: TObject);
    procedure SaveFromClipBrd1Click(Sender: TObject);
  private
    FFileName: string;
    FUpdating: Boolean;
    FDragOfs: Integer;
...
procedure TEditorMainForm.SaveFromClipBrd1Click(Sender: TObject);
var
  LFileStream: TFileStream;
begin
  if SaveDialog.Execute then
  begin
    LFileStream:= TFileStream.Create(SaveDialog.FileName, fmCreate and fmShareExclusive);
    try
      SaveClipboardToStream(LFileStream);
    finally
      LFileStream.Free;
    end;
  end;
...
procedure TEditorMainForm.SavetoClipBrd1Click(Sender: TObject);
var
  LFileStream: TFileStream;
begin
  if OpenDialog.Execute then
  begin
    LFileStream:= TFileStream.Create(OpenDialog.FileName,fmOpenRead);
    try
      LFileStream.Position := 0;
      LoadClipboardFromStream(LFileStream);
    finally
      LFileStream.Free;
    end;
  end;
...
procedure TEditorMainForm.WMCopyData(var Msg: TMessage);
begin
