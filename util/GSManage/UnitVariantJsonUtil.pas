...
uses System.Classes, Dialogs, System.Rtti,
  ...
  UnitGSFileRecord,
  mORMotHttpClient, Winapi.ActiveX, Generics.Collections, UnitBase64Util, UnitMustacheUtil;
...
uses UnitMakeReport, UnitStringUtil, StrUtils, SynMustache, UnitElecServiceData;
...
var
  ...
  LEKind_Src, LEKind_Dest: TEngineerKind;
...
begin
  ...
  LEKind_Src := String2EngineerKind(ASrc.EngineerKind);

  for i := 0 to AItemList.Count - 1 do
  begin
    LItem := AItemList.Items[i];
    LIIT_Dest := LItem.InvoiceItemType;
    LEKind_Dest := LItem.EngineerKind;

    if ((LEKind_Src = ekNone) or (LEKind_Src = LEKind_Dest)) and (LIIT_Src = LIIT_Dest) and
...
  AItem.EngineerKind := String2EngineerKind(ADoc.EngineerKind);
...
  ADoc.EngineerKind := EngineerKind2String(AItem.EngineerKind);
