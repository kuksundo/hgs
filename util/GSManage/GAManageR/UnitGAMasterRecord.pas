...
  UnitVesselData, UnitEngineMasterData, UnitGSTriffData, UnitGAServiceData;
...
procedure InitCompanyMasterClient(AExeName: string);
procedure InitFSM;
...
uses SysUtils, mORMotSQLite3, Forms, TodoList, VarRecUtils, UnitGAVarJsonUtil,
  UnitFolderUtil;
...
begin
  LStr := GetSubFolderPath(ExtractFilePath(AExeName), 'db');
  LStr := EnsureDirectoryExists(LStr);
  LStr := LStr + 'CompanyMaster.sqlite';

  MasterModel := CreateCompanyMasterModel;
  g_MasterDB:= TSQLRestClientDB.Create(MasterModel, CreateCompanyMasterModel,
    LStr, TSQLRestServerDB);
  TSQLRestClientDB(g_MasterDB).Server.CreateMissingTables;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      LStr := LStr + GSEngineerType2String(AItem.GSEngineerType);
