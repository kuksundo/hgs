...
type
  ...
  public
    ...
    procedure ColumnClick(Index: Integer);
...
begin
  inherited;
  if Integer(Message.WParam) = FSortColumn then
  begin
    FSortColumn := -1;
    FSortOrder := soNone;
  end;
...
begin
  ...
  if not (csDesigning in ComponentState) and
    ...
    with PLVItem(Message.LParam)^ do
      if FSortColumn = iSubItem then
        CustomSort(nil, FSortColumn - 1);
end;

procedure TListViewEx.ColumnClick(Index: Integer);
var
  HDItem: THDItem;
  HDNotify: THDNotify;
begin
  FillChar(HDItem, SizeOf(HDItem), 0);
  if Header_GetItem(FHeaderHandle, Index, HDItem) then
  begin
    with HDNotify do
    begin
      Hdr.hwndFrom := FHeaderHandle;
      Hdr.code := HDN_ITEMCLICK;
      Item := Index;
      PItem := @HDItem;
    end;
    SendMessage(Handle, WM_NOTIFY, 0, Integer(@HDNotify));
  end;
end;
