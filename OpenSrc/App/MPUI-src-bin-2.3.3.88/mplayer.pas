  ...
  Tmplayer = class
   ...
   private
    ...
    type TMplayerProcess=record
      ...
      ClientProcess,ReadPipe,WritePipe:THandle;
      FirstChance:boolean;
...
begin
  if (Fstreaminfo.DurationMP <> Fstreaminfo.DurationExt) and
     (Fstreaminfo.DurationExt > 0) then
    Result := (value * Fstreaminfo.DurationMP + (Fstreaminfo.DurationExt shr 1))
               div Fstreaminfo.DurationExt
  else
    Result := value;
end;
...
begin
  if (Fstreaminfo.DurationMP <> Fstreaminfo.DurationExt) and
     (Fstreaminfo.DurationMP > 0) then
    Result := (value * Fstreaminfo.DurationExt +(Fstreaminfo.DurationMP shr 1))
              div Fstreaminfo.DurationMP
  else
    Result := value;
end;
...
begin
  ...
  if DelayPositionQueries = 0 then begin
    if ((status = Splaying) or ((status = sPaused) and DidFramestep)) and running then
      if (FSecondPosInvalid) or (Fstreaminfo.DurationMP = 0)  then begin
        if FStreamInfo.stream.length <= 0 then
...
//       doonlog('autoupdate to ' + inttostr(FSecondPosExt+1));//-d-
       if Fstreaminfo.DurationExt > 0 then begin
        FPercentpos := (int64(FSecondPosExt*100000) + (Fstreaminfo.DurationExt shr 1)) div Fstreaminfo.DurationExt;
...
begin
    case FAspect of
      ...
      2:Result :=  16/9;
      3:Result :=  2.35;
      else Result := Fstreaminfo.Video.Autoaspect;
    end;
end;
...
begin
  result := DeinterlaceCmd(Mode, Alg, DeintDVD, Fstreaminfo.IsDvd, FstreamInfo.Video.Interlaced);
end;
function Tmplayer.CacheSizeCmd(sFile: string): integer;
var    drivestring :string; cacheType : integer;
begin
  ...
  if isvideofileInDrive(sFile) or isvideoFileDVD(sFile) then begin
    drivestring :=  '@:\' + sFile;
    case GetDriveType(PChar(drivestring)) of
...
begin
  result := ansipos(str,uppercase(Fstreaminfo.FileFormat))>0;
end;
...
begin
  ...
  if not check then begin
    ...
    if isVideofileMpaudio(sFile) then begin
      fstreaminfo.IsMp3 := GetMp3Info(sFile,FMp3Info);
      if (fstreaminfo.IsMp3) and (fmp3info.Duration > 0) then begin
        fstreaminfo.DurationExt := fmp3info.Duration;
      end;
  ...
  if not check then begin
    Fstreaminfo.IsDvd := IsDvd;
    Fstreaminfo.IsDvdNav := IsDvdnav;
    Frenderinfo.IsPlayingDVDMenu := IsDvdnav;
    Fstreaminfo.stream.length := filelength;
    Fstreaminfo.Video.Interlaced := Interlaced;
    if not IsDvd then
...
begin
  ...
  Fstreaminfo.FileName := Fmediafile;
  ...
  if cmdOk and (not fromcue) then begin
    ...
    CreatePipe(pr.ReadPipe,DummyPipe1,@sec,0);
    ...
    CreatePipe(DummyPipe2,pr.WritePipe,@sec,0);
    // create process
...
begin
  ...
  if not Reload then begin
    if (Fstreaminfo.DurationMP = 0) or SecondPosInvalid then begin
    //    if StreamInfo.stream.length > 0 then begin ; for testing
      if FStreamInfo.stream.length <= 0 then begin
...
begin
   Result := false;
   if Fstreaminfo.IsDvd then begin
...
//      if (Fstreaminfo.IsMp3) and (Method = 4) then
...
begin
  if (FStatus = sPlaying) and FStreaminfo.IsDvdNav then begin
...
begin
  ...
  if forced or (Value <> FSecondPosExt) then begin
    FSecondPosExt := value;
    if FfileLoaded ='' then
      if (FsecondPosExt > 5) and (FSecondPosExt > (Fstreaminfo.DurationExt -5)) then
...
begin
  if time > (Fstreaminfo.DurationMP + 10) then
...
begin
  text := '';
  if isvideofilempg(StreamInfo.FileName) or Fstreaminfo.IsDvd then
    case id of
    0..31:
      text := 'MPEG';
    128..159:
...
var r,i,j,p:integer; c:char;
  ...
  begin
    ...
    if Streaminfo.Video.Width = 0 then begin
      if not getResolution('] ', line, i,j) then exit;
      FStreaminfo.Video.Width := i;
      FStreaminfo.Video.Height := j;
    end;
  ...
  begin
    ...
    if FStreaminfo.HaveVideo and (cmdinisent = 0) then
      sendcmdsInit(1)
    else begin
      sendcmdsInit(2);
    end;
  ...
  begin
    ...
    if Result then begin
      if failed then
        f := Fstreaminfo.DurationMP
      else
        getStringValueFloat(Line,f);

      if (not fromId) and (fstreaminfo.DurationMP <= 0) then begin
        FUseSeekbystream := true;
        DoOnlog('Time Duration is 0, using alternate seek method (stream pos)');
        if Fstreaminfo.stream.length > 0 then
          FSecondPosInvalid := true;
      end else begin
        Fstreaminfo.DurationMP := Correctround(f);
        CorrectLenght := false;
        if not(fromId) and (Fstreaminfo.DurationMP > 0)  then begin
          CorrectLenght := abs(Fstreaminfo.DurationMP -  Fstreaminfo.DurationExt)
                           > (Fstreaminfo.DurationMP div 100);
        end;
        if (Fstreaminfo.DurationExt <= 0) then
          Fstreaminfo.DurationExt := Fstreaminfo.DurationMP
        else begin
          if (Fstreaminfo.IsMp3 and (FMp3Info.VbrBitRate >0)) then begin
        ...
        Fstreaminfo.Durationstring:=SecondsToTimeHMS(Fstreaminfo.DurationExt);
      end;
...
//        setstatus(sPlaying);// set by sendplaypause
  ...
  begin
    ...
    if Result then
      FStreamInfo.FileFormat:=Copy(Line,1,p-1);
  end;
  ...
  begin
    ...
    if Copy(Line,9,4)='vide' then
      FStreamInfo.Video.Decoder:=Copy(Line,p+2,length(Line))
    else if Copy(Line,9,4)='audi' then
      FStreamInfo.Audio.Decoder:=Copy(Line,p+2,length(Line))
    else Result:=false;
  ...
  begin
   with fStreaminfo.Video do begin
  ...
  begin
    ...
    if Copy(Line,10,4)='vide' then
      FStreamInfo.Video.Codec:=Copy(Line,p+2,length(Line)-p-2)
    else if Copy(Line,10,4)='audi' then
      FStreamInfo.Audio.Codec:=Copy(Line,p+2,length(Line)-p-2)
    else Result:=false;
  end;
  ...
  begin
    ...
    P:=0; while (P<9)
          ...
          do inc(P);
    FStreamInfo.ClipInfo[P].Key:='Title';
    if FStreamInfo.ClipInfo[P].Value<>Line then begin
      FStreamInfo.ClipInfo[P].Value:=Line;
      //InfoForm.UpdateInfo;
    end;
    Result:=true;
  end;
        ...
        if tit > 0 then begin // is title property
          ...
          if pos > 0 then begin
            ...
            if ansipos('DISC_ID=', line) > 0 then begin
              Fstreaminfo.ClipInfo[0].Key := 'DISC ID';
              Fstreaminfo.ClipInfo[0].Value := getStringValue(line);
            end else if ansipos('VOLUME_ID=', line) > 0 then begin
              Fstreaminfo.ClipInfo[1].Key := 'VOLUME ID';
              Fstreaminfo.ClipInfo[1].Value := getStringValue(line);
            end;
        end;
      Result := true;
      end;
  ...
  begin
    ...
    if ansname = 'switch_audio' then begin
      ...
      if getStringValueInt64(line,toint64) then begin
        if ansname[8]='s' then
          Fstreaminfo.stream.start := toint64
        else
          Fstreaminfo.stream.length := toint64;
        result := true;
      end;
...
(*  function checkDVDErrors : boolean;
  begin
    Result := false;
    if fstreaminfo.IsDvdNav then begin
  ...
  *)
  if (length(Line)>11) then begin
    ...
    if ((Line[j-2]='A') OR (Line[j-2]='V')) AND (Line[j-1]=':') then begin
      if FSecondPosInvalid or (Fstreaminfo.DurationMP  = 0) then exit;

      if fstreaminfo.Havevideo  then
      ...
      if p > 0 then begin
        i := (p*10000 + (Fstreaminfo.DurationMP shr 1)) div Fstreaminfo.DurationMP;

        p := (p + 5 ) div 10;

        if (p > (Fstreaminfo.DurationMP+10)) or
           ((secondpos = -1) and (p > 10)) then begin
           if not Fstreaminfo.IsDvdNav then begin
             Fpercentpos := 0;
             FSecondPosMP := 0;
             FSecondPosExt := 0;
             FSecondPosInvalid := true;
  ...
  *)
  ...
  // check second position indicator  ; visenri
   if Copy(Line,1,18)='ANS_TIME_POSITION=' then begin
    ...
    if (secondposReal>=0.01) then begin
          ...
          if (i > (Fstreaminfo.DurationMP +10)) or
            ...
            if Fstreaminfo.DurationMP > 0 then begin
              i := (Correctround(secondposReal * 100000.0) + (Fstreaminfo.DurationMP shr 1)) div Fstreaminfo.DurationMP;
              DoOnPercentPosition(i);
            end;
          end;
    end;
  ...
  // check percent_position indicator (hidden from log)
    ...
    if Copy(Line,1,21)='ANS_PERCENT_POSITION=' then begin
     ...
     if not(FSecondPosInvalid or (Fstreaminfo.DurationMP = 0)) or
        (StreamInfo.stream.length > 0) then exit;

     Val(Copy(Line,22,4),i,r);
     if (r=0) AND (i>=0) AND (i<=100) then begin
       p:=i*1000;
       if Fstreaminfo.DurationMP > 0 then
         i := (p*Fstreaminfo.DurationMP) div 100000
       else
         i := 0;
    ...
    if Copy(Line,1,15)='ANS_stream_pos=' then begin
      ...
      if not(SecondPosInvalid or (Fstreaminfo.DurationMP = 0)) or
         (StreamInfo.stream.length <= 0) then exit;

      Fstreampos := strtoint64(trim(Copy(Line,16,20)));

      i := ((Fstreampos - Fstreaminfo.stream.start ) * Fstreaminfo.DurationMP)
           div Fstreaminfo.stream.length;
      p := ((Fstreampos - Fstreaminfo.stream.start ) * 100000)
           div Fstreaminfo.stream.length;
...
begin
  ...
  if RestartNeeded = true then
    if RestartEnabled then
      if FirstOpen and FstreamInfo.IsDvd then
...
begin
  if (ValidFileInfo and FstreamInfo.Havevideo) or
...
begin
  Result := (length(Mediafile)<>0) and FstreamInfo.Valid;
end;
