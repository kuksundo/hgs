...
type
  TfrmMain = class(TVssDockForm)
    ...
    //for testing speeed of components
  ...
  private
    ...
    procedure DrawItemText(ACanvas: TCanvas;ARect: TRect;Text: string; shortcut : string; ena : boolean; sel : boolean; acc : boolean);
    procedure setMnuDrawEvents(Mnu : Tmenuitem);
    procedure advmnuDrawItem(Sender: TObject; ACanvas: TCanvas;
      ARect: TRect; State: TOwnerDrawState);
    procedure mnuMeasureItem(Sender: TObject; ACanvas: TCanvas;
...
uses Locale, Config, URL,NumUtils, dwmApi, CustomFormsHelper,
     ...
     VssAppVersion, VssThisAppVersion;
...
begin
  ...
  imgaudio.Left := 440;
  Ltime.Left := 425;  //IDE moves this controls from time to time ¿?

  imgstatus.Tag := -1;
  imgAudio.Tag := -1;

  //SetDoubleBuffer(ControlPanel);
  //ControlPanel.ControlStyle:=ControlPanel.ControlStyle+[csOpaque];
//  SetDoubleBuffer(volslider);
//  SetDoubleBuffer(VolFrame);
//  SetDoubleBuffer(Volboost);
  SetDoubleBuffer(OuterPanel);
//
//  SetDoubleBuffer(Innerpanel);
  SetDoubleBuffer(Pstatus);
  setdoublebuffer(PbuttonsPlay);
  SetDoubleBuffer(PButtons);
  ////Bplay.ControlStyle:=bPlay.ControlStyle+[csOpaque];
...
//AllocConsole;
  ...
  setMnuDrawEvents(Mainmenu.Items);
  setMnuDrawEvents(Mpopup.Items);
  ...
  seekbar.SetBounds(4, 1 ,self.ClientWidth-8, 20);
  seekbar.SliderLineHeight := 8;
  ...
  speedbar.SetBounds(bnext.Left+ bnext.Width ,
                    pButtonsplay.Top + bnext.Top + (bnext.Height div 2) - 10,
                    200, 20);
  speedbar.Min := -18;
  speedbar.Max := 22;
  speedbar.Position := 0;
  speedbar.Enabled := true;
  speedbar.SliderLineHeight := 4;
...
begin //
  timerVis.Enabled := Playing and (not mpo.streaminfo.HaveVideo) and visEffects.Enabled;
  Logo.Visible := (not Playing);
...
//    Ord('8'):   OMHelp.CheckMenuDropdown;
   ...
   case Key of
    ...
    VK_NUMPAD9 : inczoom(1); // zoom keys // visenri
    VK_NUMPAD5 : inczoom(0);
    VK_NUMPAD1 : inczoom(-1);

    VK_NUMPAD6 : incaspectf; // zoom keys // visenri
...
begin
  //resize Timage bitmap if needed
  ...
  if step then begin
    VisEffects.RenderNewFrame; // render new frame
  end else begin
    VisEffects.DrawToBitmap; //redraw existing one(paused window resize)
  end;
  //VisProc.DrawInBm(imgvis.Picture.Bitmap);
  //imgvis.Canvas.Draw(0,0,visproc.bmBG);


//  imgvis.Canvas.CopyRect(rec,visproc.bmBG.Canvas ,rec);
  //bitblt(imgvis.Canvas.Handle,0,0,rec.Right,rec.Bottom,
  //       visproc.bmBG.Canvas.Handle,0,0, SRCCOPY ) ;
  //imgvis.Invalidate;
end;
...
begin
    pos := ((pstatus.Width*3) div 5) - lzoom.Width;
    ...
    setvis(bplaylist, pButtonsPlay.Width < (pbuttons.Left+bplaylist.Left));
    setvis(Bstreaminfo, pButtonsPlay.Width < (pbuttons.Left+Bstreaminfo.Left));

    speedbar.Left  :=  bnext.Left + bnext.Width +5;
...
begin
  ...
  imagery.GetBitmap(Msettings.ImageIndex,Bsettings.Glyph);
  imagery.GetBitmap(Mstreaminfo.ImageIndex,Bstreaminfo.Glyph);
  imagery.GetBitmap(MCompact.ImageIndex,Bcompact.Glyph);
...
begin
  ...
  lastseekbarpos := -1;
  //QueryPosition;
  //delay position queries to avoid showing old time
  //DelayPositionqueries := 6;
  //Unpaused;
end;
...
var
    ...
    actVideo : boolean;
begin
//  if (not msizeany.Checked) and (self.WindowState = wsMaximized) then begin
//    ControlledResize:=1;
...
//  end;
  ...
  //  I := GetMonitorIndex(Monitor.MonitorNum);
  //  if I <> -1 then
  //    with screen do begin
  //      if (Monitors[I].Width + Monitors[I].Left) < (px + sx) then //check outside screen
  //        px := (Monitors[I].Width + Monitors[I].Left) -sx;
  //      if (Monitors[I].Height + Monitors[I].top) <(py +sy) then
  //        py := (Monitors[I].Height + Monitors[I].top) -sy;
  //      if px < Monitors[I].Left  then //check outside screen after new pos
  //        px := Monitors[I].Left;
  //      if py < Monitors[I].Top  then
  //        py := Monitors[I].Top;
  //    end;
...
procedure TfrmMain.Update_MOpenDrive;
  ...
  begin
    if GetVolumeInformation(PAnsiChar(DriveChar + ':\'),
...
begin
  ...
  //LockWindowUpdate(self.Handle); //ORIGINAL LOCK
  ...
  //LockWindowUpdate(0); //ORIGINAL LOCK
end;
...
begin
  if (mpo.Status<>sPlaying) AND (mpo.Status<>sPaused)
    then LTime.Caption:=''
    else LTime.Caption:=SecondsToTimeHMS(mpo.SecondPos)+
...
begin
  ...
  //ShowOwnedPopups(Application.Handle, show);
...
begin
  ...
  {$IFDEF APPLICATIONFORMSFIX}
  if Minimizing or RestoringFromMinimized then
  ...
  if RestoringFromMinimized  then begin
...
procedure TfrmMain.DrawItemText(ACanvas: TCanvas;ARect: TRect;Text: string; shortcut : string; ena : boolean; sel : boolean; acc : boolean);
var
 seppos : integer;
 lefttext : string;
 righttext : string;
 flags : Cardinal;
begin
  seppos := ansipos(#9, text);

  righttext := shortcut;

  if seppos > 0 then begin
    lefttext := copy(text,1,seppos-1);
    if righttext = '' then
      if length(text) > seppos then
        righttext := copy(text, seppos+1,length(text)-seppos)
  end else begin
    lefttext := text;
  end;
  flags := DT_VCENTER or DT_SINGLELINE or DT_NOCLIP;
  if not acc then
    flags := flags or DT_HIDEPREFIX;

  if ena then begin
    DrawText(ACanvas.Handle, PChar(lefttext), -1, ARect, DT_LEFT or flags)
  end else begin
  ...
  if righttext <> '' then begin
    DrawText(ACanvas.Handle, PChar(righttext), -1, ARect, DT_RIGHT  or flags);
  end;
// acanvas.TextRect()
end;
...
begin
  p:= mnu.Parent;
  if (p <> nil)  then begin
    //if not( mnu.GetParentComponent is Tmainmenu)then
      mnu.OnAdvancedDrawItem  :=  advmnuDrawItem;
      mnu.OnMeasureItem :=  mnuMeasureItem;
  end;
...
procedure TfrmMain.advmnuDrawItem(Sender: TObject; ACanvas: TCanvas;
  ARect: TRect; State: TOwnerDrawState);
var
  mnuItem : TmenuItem;
  iconsize : integer;
  ...
  menColor : Tcolor;
  hotlight : boolean;
  mainmenu : boolean; //main root menu
  popupmenu : boolean; //main popup menu
  lbRect : Trect;
  Isflat : Cardinal;
  dsIcon : Tdrawingstyle;
  rrect : Trect;
begin
  rrect.Right :=-1;

  mnuItem := Sender as TmenuItem;
  iconsize := imagery.Height;

  hotlight := false;
  mainmenu := isMenuParentClass(mnuItem, Tmainmenu);
  popupmenu := isMenuParentClass(mnuItem,Tpopupmenu);

  menColor := ACanvas.Brush.Color;


  if popupmenu then begin
    lbRect := ARect;
    ARect.Left  := ARect.Left  + PopupBarWidth + PopupBarSpace;
    lbrect.Right := arect.Left - PopupBarSpace;

    if PopupBitmap = nil then
      PopupBitmap := Tbitmap.Create();
    PopupBitmap.Height := PopupHeight;
    PopupBitmap.Width := PopupBarWidth;

    if not PopupCanvasDrawn then begin
      DrawVerticalGradient(Popupbitmap,PopupclStart,PopupclEnd);
      DrawVerticalText(Popupbitmap,'MPUI-Ve', AppVersion.ProductVersion.GetString);
      PopupCanvasDrawn := true;
    end;

    ACanvas.CopyRect(lbrect, PopupBitmap.Canvas, lbrect);

    lbrect.Left := lbrect.Right;
    lbrect.Right := lbrect.Left + PopupBarSpace;
    ACanvas.Brush.Color := clBtnHighlight;
    ACanvas.Fillrect(lbRect);
  end;


  if mainmenu then begin
    if not SystemParametersInfo(SPI_GETFLATMENU,0,@Isflat,0) then begin
      isflat := 0;
    end;
    //if colortorgb(clmenubar) <> $A5BFC2 then*)
    if isflat <> 0 then begin
      menColor := clMenuBar;
    end;


    if self.Color <> clbtnface then begin
      menColor := self.Color;
      Acanvas.Font.Color := self.Font.color;

      if (mnuitem.Parent.Items[mnuitem.Parent.Count-1]= mnuItem) then begin
    //      logform.AddLine((mnuItem.Name));
        rrect := Arect;
        rrect.Right := self.ClientWidth + (self.Width -self.ClientWidth) div 2;
        rrect.Left := arect.Right;
        if rrect.Right <= rrect.Left then
          rrect.Right := -1;
      end;
    end;

    if odHotLight in state then begin
      selcolor := ColorToRGB(Acanvas.Font.Color) xor $FFF0F0;
      hotlight := true;
    end else
      selcolor := ColorToRGB(Acanvas.Font.Color) xor $FFF0F0
  end else begin
  ...
  if (odSelected in State) or hotlight  then
    ACanvas.Brush.Color := selcolor
  else
    ACanvas.Brush.Color := mencolor;
  ...
  iconIndex:= -1;
  if mnuItem.ImageIndex >= 0 then begin
    iconIndex := mnuItem.ImageIndex;
    if iconindex = ICON_CHECK_OFF then
      if mnuItem.Checked then
        iconindex := ICON_CHECK_ON
  end else
    if mnuItem.RadioItem  then
      if mnuItem.Checked then
        iconindex := ICON_RADIO_ON
      else
        iconindex := ICON_RADIO_OFF
    else
      if mnuItem.Checked  then
        iconindex := ICON_CHECK;

  if iconindex >= 0 then begin
    if mnuItem.enabled then begin
      dsIcon := dsNormal;
    end else begin
      dsIcon := dsSelected;
    end;

    imagery.Draw(ACanvas,ARect.Left +1,ARect.Top + (arect.Bottom - arect.Top -iconsize) div 2 ,
              iconindex,dsIcon,itImage);

    iconRect := arect;
    iconRect.left := ARect.Left + 0;
    iconRect.Right := ARect.Left + 2 + iconsize;
    //normal icons
    if (mnuItem.ImageIndex >= 0) and (iconindex <> ICON_CHECK_ON) and  mnuItem.Checked then begin
      acanvas.Brush.Color := clred;
      acanvas.Pen.Color := clRed;
      acanvas.Pen.Mode := pmMerge;
      acanvas.Rectangle(iconRect);
    end;
  end;
  ...
  acanvas.Brush.Style := bsClear; //transparent text

  //ACanvas.Font.Style := [fsBold];
  //acanvas.Font.Color := clred;
  arect.Right := arect.Right - 12;
  if ( mnuItem.GetParentComponent is Tmainmenu)then
    Arect.Left := Arect.Left + 6
  else
    Arect.Left := Arect.Left + iconsize+6;

  DrawItemText(ACanvas,ARect,
      mnuItem.caption, shortcuttotext(mnuItem.ShortCut),
      mnuItem.Enabled, (odSelected in State), not(odNoAccel in State));

  if rrect.right > 0 then begin
    acanvas.Brush.Color := mencolor;
    acanvas.FillRect(rrect);
  end;
end;
procedure TfrmMain.mnuMeasureItem(Sender: TObject; ACanvas: TCanvas;
  var Width, Height: Integer);
var
  mnuItem : TmenuItem;
  popupmenu : boolean;
begin
  ...
  popupmenu := isMenuParentClass(mnuItem,Tpopupmenu);
  if popupmenu then begin
    if mnuItem = self.MPopup.Items[0] then
      PopupHeight := 0;
    width := width + PopupBarWidth + PopupBarSpace;
