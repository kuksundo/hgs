...
type
  ...
  protected
    ...
    function Validate(_Condition: Boolean): Boolean;
  ...
  IdzTextControlDateValidator = interface ['{80C45A30-6137-4AA7-9AD0-198FFF36908A}']
    ...
    function TryGetValue(out _Value: TDate): Boolean;
  ...
  IdzCheckboxValidator = interface ['{5B2974A5-A433-4874-BD3E-CC86E7489AA3}']
    ...
    function ValidateIsUnchecked: Boolean;
    function ValidateIsNotGrayed: Boolean;
...
begin
  s := doGetText;
  Result := (s = '') or TryStr2Float(s, Value, #0);
  Validate(Result);
end;
...
begin
  s := doGetText;
  Result := (s = '') or TryStr2Int(s, Value);
  Validate(Result);
end;
...
begin
  s := doGetText;
  if TryStr2Date(s, dt) then begin
    Result := Trunc(dt);
  end else begin
...
begin
  s := doGetText;
  if TryStrToTime(s, dt) then begin
    Result := Frac(dt);
  end else begin
    if _FocusControl then
      doFocusControl;
...
begin
  ...
  if Validate(Result) then
    FValidator.SetErrorMessage(FControl, '')
  else
    FValidator.SetErrorMessage(FControl, Format(_('Value must be between %.2f and %.2f.'), [_MinValue, _MaxValue]));
end;
...
begin
  ...
  if Validate(Result) then
    ...
    FValidator.SetErrorMessage(FControl, Format(_('Value must be between %.2f and %.2f or empty.'), [_MinValue, _MaxValue]));
end;
...
begin
  ...
  if Validate(Result) then
    ...
    FValidator.SetErrorMessage(FControl, Format(_('Value must be greater or equal %.2f.'), [_MinValue]));
end;
...
begin
  ...
  if Validate(Result) then
    ...
    FValidator.SetErrorMessage(FControl, Format(_('Value must be greater than %.2f.'), [_MinValue]));
end;
...
begin
  ...
  if Validate(Result) then
    ...
    FValidator.SetErrorMessage(FControl, Format(_('Value must be less or equal %.2f.'), [_MaxValue]));
end;
...
begin
  ...
  if Validate(Result) then
    ...
    FValidator.SetErrorMessage(FControl, Format(_('Value must be less than %.2f.'), [_MaxValue]));
end;
...
begin
  ...
  if Validate(Result) then
    ...
    FValidator.SetErrorMessage(FControl, _('Must be a floating point value.'));
end;
...
begin
  ...
  if Validate(Result) then
    ...
    FValidator.SetErrorMessage(FControl, _('Must be a floating point value or empty.'));
end;
...
begin
  s := doGetText;
  Result := TryStr2Int(s, Value);
  if Validate(Result) then
    FValidator.SetErrorMessage(FControl, '')
  else
    FValidator.SetErrorMessage(FControl, _('Value must be a valid integer number.'));
end;
...
begin
  ...
  if Validate(Result) then
    ...
    FValidator.SetErrorMessage(FControl, Format(_('Value must be between %d and %d.'),
      [_MinValue, _MaxValue]));
end;
...
begin
  ...
  if Validate(Result) then
    ...
    FValidator.SetErrorMessage(FControl, Format(_('Value must be smaller than %d.'), [_MaxValue]));
end;
...
begin
  ...
  if Validate(Result) then
    ...
    FValidator.SetErrorMessage(FControl, Format(_('Value must be greater than %d.'), [_MinValue]));
end;
...
begin
  ...
  if Validate(Result) then
    ...
    FValidator.SetErrorMessage(FControl, Format(_('Value must be greater or equal %d.'), [_MinValue]));
end;
...
begin
  ...
  if Validate(Result) then
    ...
    FValidator.SetErrorMessage(FControl, _('Value must not be empty.'));
end;
...
begin
  ...
  if Validate(Result) then
    ...
    FValidator.SetErrorMessage(FControl, _('No entry selected.'));
end;
...
function TCheckboxValidator.ValidateIsChecked: Boolean;
begin
  Result := Validate(Checkbox.Checked);
end;

function TCheckboxValidator.ValidateIsNotGrayed: Boolean;
begin
  Result := Validate(Checkbox.State <> cbGrayed);
end;

function TCheckboxValidator.ValidateIsUnchecked: Boolean;
begin
  Result := Validate(not Checkbox.Checked);
end;
...
begin
  ...
  try
    ...
    for i := 1 to Length(Full) do
      if not (CharInSet(Full[i], ['0'..'9', '.'])) then
        Exit;
    for i := 1 to 4 do begin //FI:W528
      s := ExtractStr(Full, '.');
      if (Length(s) > 3) or not TryStrToInt(s, Value) or (Value < 0) or (Value > 255) then
        Exit;
    end;
    Result := (Full = '');
  finally
