...
{$IFDEF DELPHIXE2_UP}
  THandleCast = THandle;
{$ELSE}
  THandleCast = Integer; // Delphi < XE2 wrongly declares the handle parameter
                         // to THandleStream.Create as signed integer while
                         // it really should be unsigned. Delphi XE2 corrects
                         // this but in order to compile with both version we
...
begin
  if THandleStreamHack(Self).FHandle <> THandleCast(INVALID_HANDLE_VALUE) then
...
begin
  if _Count <> 0 then begin
    ...
    if (Written <> _Count) then begin
      ...
      // be reaplaced with a single % by the format function.
