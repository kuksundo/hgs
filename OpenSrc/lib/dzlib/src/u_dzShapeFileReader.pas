  ...
  public
    ...
    type
      TOnFileHeader = procedure(_Sender: TObject;
        ...
        _XMax: Double; _YMax: Double; _ZMax: Double; _MMax: Double) of object;
...
procedure TShapeFileReader.doOnFileHeader(_FileCode, _FileLength: Integer;
  _Version: Integer; _ShapeType: TShapeTypeEnum;
  _XMin, _YMin, _ZMin, _MMin, _XMax, _YMax, _ZMax, _MMax: Double);
begin
...
var
  ...
  NumParts: LongWord;
begin
  doReadMultiPartShape(_Stream, _RecordHeader, _Box, _Parts, NumPoints, NumParts);
end;
...
var
  ...
  Offset: Integer;
begin
  doReadMultiPartShape(_Stream, _RecordHeader, Box, Parts, NumPoints, NumParts);
  _Box.XMin := Box.XMin;
  _Box.XMax := Box.XMax;
  _Box.YMin := Box.YMin;
  _Box.XMax := Box.YMax;
  ...
  _Stream.ReadBuffer(ZValues, Length(ZValues) * SizeOf(ZValues[0]));
  ...
  if _RecordHeader.ContentLength =
    ...
    _Stream.ReadBuffer(MValues, Length(MValues) * SizeOf(MValues[0]));
  end else
...
var
  ...
  begin
    // todo: Implement
    _Stream.Seek(_RecordHeader.ContentLength - SizeOf(LongWord), soCurrent);
  end;
...
begin
  ...
  try
    ...
    while not Stream.EOF do begin
      ...
      Stream.Read(ShapeType, SizeOf(ShapeType));
