...
type
  ...
  private
    ...
    FParser: TParser;
  protected
    // Method of the new function, which sums the parameters from left and right
    // and multiplies the result by 3:
...
begin
  ...
  FParser.AddFunction('Triple', FTripleHandle, fkMethod,
    FunctionMethod(TripleMethod), True, False, vtDouble);

  {
    FunctionMethod here needs only the one parameter:
...
begin
  ...
  AValue := Convert(LValue, vtDouble).Float64;
  BValue := Convert(RValue, vtDouble).Float64;

  AssignDouble(Result, (AValue + BValue) * 3);
end;
