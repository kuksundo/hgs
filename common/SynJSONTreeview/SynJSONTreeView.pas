...
begin
  if (aJSON = Unassigned) then begin
    ...
    if (lValue = '') then
      Exit;
    if (VariantToUTF8(lValue)[1] <> '[') then
      if (VariantToUTF8(lValue)[1] <> '{') then
        lValue := '{' + lValue + '}';
    ...
    lJSON := aJSON;
  // if parent is an array add the item
  if (Node.fJSONType in [jtnArray, jtnArrayContent]) then begin
    TDocVariantData(Node.Value).AddItem(lJSON);
    ProcessObject(Node, _CopyFast(lJSON), '');
  end
  else begin
    lCount := lJSON._Count;
    TDocVariantData(Node.Value).AddOrUpdateObject(lJSON);
    ...
    dvObject:
      if not VarIsEmpty(aNameOrIndex) then begin
        ...
        if (VariantToIntegerDef(aNameOrIndex, -1) <> -1) then begin
          ...
          // if aNameOrIndex is null  then the name is the new index
          if (aNameOrIndex = '') then
            Result.Name := IntToStr(aNode.Count-1) // the node already been added
          else
