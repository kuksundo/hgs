...
type
  ...
  protected
    ...
    {
      Handler of TParser.OnFunction event is needed to implement the new function,
      it returns True when succeeded or False when it is not, the result lies in
      the output Value parameter. LValue and RValue are the function parameters
      from left and right and only meaningful when function requires either one
      of them or both. For example if function does not require parameters then
      the LValue and RValue parameters contain no data. ParameterArray is the array
      for the function which requires a number of parameters; if it does not than
      the ParameterArray is empty:
    }
...
function TMain.Custom(const AFunction: PFunction; const AType: PType;
  out Value: TValue; const LValue, RValue: TValue;
  const ParameterArray: TParameterArray): Boolean;
var
  AValue, BValue: Double;
begin
  {
    All input parameters and the output Value parameter are kept in TValue structure.
    TValue structre allows to decribe a variable which may have one of the following
    types:
      - Byte
      - Shortint
      - Word
  ...
  Result := AFunction.Handle^ = FMultiplyHandle;
  if Result then
  begin
    if Length(ParameterArray) <> 2 then
      raise Exception.CreateFmt('Function "%s" requires two parameters!', [AFunction.Name]);

    AValue := Convert(ParameterArray[0].Value, vtDouble).Float64;
    BValue := Convert(ParameterArray[1].Value, vtDouble).Float64;

    AssignDouble(Value, AValue * BValue);
  end;
end;
...
begin
  ...
  // Assign OnFunction event handler:
  FParser.OnFunction := Custom;

  {
    ...
    - Whole: defines if the function behaves itself like a separator during parsing
      the formula. For example - Multiply function "*" does not need to be enclosed
      by space symbol: "2*2", "2 *2", "2* 2", "2 * 2" - all the expressions will
      succeed (Whole is False for the multiply function). Continuing the sample -
      if Whole for the multiply function "*" is True then only the following expression
      will succeed - "2 * 2";
    - ReturnType of TValueType defines function returning type. Parameter matters
      for the decompilation methods only and may be omited.
  }

  FParser.AddFunction('Multiply', FMultiplyHandle, fkHandle,
    FunctionMethod(False, False, 2), True, False, vtDouble);

  {
    FunctionMethod here needs three parameters:
      - LParameter: defines that function "Multiply" does not require parameter
        from left;
      - RParameter: defines that function "Multiply" does not require parameter
        from right;
      - Count: defines an amount of parameters, following right after the function
