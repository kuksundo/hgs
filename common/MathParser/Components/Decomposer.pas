...
begin
  ...
  try
    ...
    if LessZero(Epsilon) then
      Builder.Append(Space + TextOperatorArray[toNegative] + Space + ValueToText(Positive(Epsilon)))
    else
      Builder.Append(Space + TextOperatorArray[toPositive] + Space + ValueToText(Epsilon));
    Result := Builder.Text;
  finally
    Builder.Free;
  end;
    ...
    try
      ...
      try
        ...
        begin
          ...
          if I < Count - 1 then
          begin
            J := Random(Length(TextOperatorArray));
            Builder.Append(Space + TextOperatorArray[TTextOperator(J)] + Space);
          end;
        end;
      finally
        Index := nil;
