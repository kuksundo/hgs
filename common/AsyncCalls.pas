{**************************************************************************************************}
{ WARNING: this is NOT the original asynccalls.pas as released by Andreas                          }
{ Altered version by Zarko Gajic: added "Cancel" to IAsyncCall                                     }
...
type
  ...
  private
    FCancelled : boolean; //zarko gajic
...
procedure TSyncCall.Cancel;
begin
  FCancelled := true; //zarko gajic
end;

constructor TSyncCall.Create(AReturnValue: Integer);
begin
  inherited Create;
  FCancelled := false; //zarko gajic
...
begin
  Result := FCancelled OR (FEvent = 0) or FFinished or (WaitForSingleObject(FEvent, 0) = WAIT_OBJECT_0); //zarko gajic
end;
...
begin
  ...
  Value := 0;
  try
    IF NOT FCancelled THEN Value := ExecuteAsyncCall; //zarko gajic
  except
...
asm
  ...
  { Detect if the finally block is called by System._HandleFinally.
    ...
    thread switch for us. This will also restore the EBP reg?ter. }
