...
type
  ...
  private
    ...
    FPingReqProc: TPingRegProc;
    FIsPingReqExec: Boolean;
    FSubscribeProc: TSubscribeProc;
    FUnSubscribeProc: TUnSubscribeProc;
    FIsSubscribeExec: Boolean;
    FIsUnSubscribeExec: Boolean;
    FTopic: AnsiString;
    FQos: integer;
  ...
  public
    ...
    procedure SetSubscribeTopic(Topic: Ansistring; RequestQoS: integer);
    property PingReqProc : TPingRegProc read FPingReqProc write FPingReqProc;
    property IsPingReqExec : Boolean read FIsPingReqExec write FIsPingReqExec;
    property SubscribeProc : TSubscribeProc read FSubscribeProc write FSubscribeProc;
    property IsSubscribeExec : Boolean read FIsSubscribeExec write FIsSubscribeExec;
    property UnSubscribeProc : TUnSubscribeProc read FUnSubscribeProc write FUnSubscribeProc;
    property IsUnSubscribeExec : Boolean read FIsUnSubscribeExec write FIsUnSubscribeExec;
...
begin
  ...
  IsPingReqExec := False;
  IsSubscribeExec := False;
  IsUnSubscribeExec := False;
...
begin
  while not Terminated do
    begin
      FCSock.Acquire;
      try
        if IsPingReqExec then
        begin
          try
            CodeSiteLog('FPingReqProc', '0');
            if Assigned(FPingReqProc) then FPingReqProc;
            CodeSiteLog('FPingReqProc', '1');
            continue;
          finally
            IsPingReqExec := False;
          end;
        ...
        if IsSubscribeExec then
        begin
          try
            CodeSiteLog('FSubscribeProc', FTopic);
            if Assigned(FSubscribeProc) then FSubscribeProc(FTopic, FQos);
            CodeSiteLog('FSubscribeProc', IntToStr(FQos));
            continue;
          finally
            IsSubscribeExec := False;
          end;
        ...
        if IsUnSubscribeExec then
        begin
          try
            CodeSiteLog('UnSubscribeProc', FTopic);
            if Assigned(FSubscribeProc) then FUnSubscribeProc(FTopic);
            CodeSiteLog('UnSubscribeProc', IntToStr(FQos));
            continue;
          finally
            IsUnSubscribeExec := False;
          end;
...
//                CodeSiteLog('FixedHeaderByte', '1');
              end;
            TMQTTRecvState.RemainingLength:
              begin
//                CodeSiteLog('RemainingLength', '0');
                RLInt := 0;
...
//                CodeSiteLog('RemainingLength', IntToStr(RLInt));
              end;
            TMQTTRecvState.Data:
              begin
//                CodeSiteLog('Data Start', IntToStr(RLInt));
                if (RLInt > 0)  then
                begin
                  SetLength(CurrentMessage.Data, RLInt);
                  RLInt := RLInt - FPSocket.RecvBufferEx(Pointer(CurrentMessage.Data), RLInt, 1000);
//                  CodeSiteLog('Data Ing', 'FPSocket.RecvBufferEx = ' + IntToStr(FPSocket.LastError) + ','+ IntToStr(RLInt));
                end;

                if ((FPSocket.LastError = 0) and (RLInt = 0)) then
                begin
                  FCurrentMsg := CurrentMessage;
//                  CodeSiteLog('Data Ing', 'ProcessMessage Start = ' +   IntToStr(FCurrentMsg.FixedHeader shr 4));
                  Synchronize(ProcessMessage);
//                  CodeSiteLog('Data Ing', 'ProcessMessage End <======');
                  CurrentMessage := Default(TUnparsedMsg);
                  FCurrentRecvState := TMQTTRecvState.FixedHeaderByte;
                end;
//                CodeSiteLog('Data End', IntToStr(FPSocket.LastError) + ','+ IntToStr(RLInt));
              end;
...
function TMQTTReadThread.IsSocketAlive(ASocket: TTCPBlockSocket): Boolean;
begin
...
//  CodeSiteLog('ProcessMessage Start', 'FHCode = ' + IntToStr(FHCode));
  case FHCode of
...
//  CodeSiteLog('ProcessMessage End', 'TMQTTRecvUtilities.MSBLSBToInt(NewMsg.Data) = ' + IntToStr(TMQTTRecvUtilities.MSBLSBToInt(NewMsg.Data)));
end;
...
procedure TMQTTReadThread.SetSubscribeTopic(Topic: Ansistring;
  RequestQoS: integer);
begin
  FTopic := Topic;
  FQos := RequestQos;
